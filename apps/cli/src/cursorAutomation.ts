...
function buildBuilderPrompt(
  ...
  const base = [
    ...
    "",
    "Scope: implement the **primary slice** from `.foundry/BUILD_SPEC.md` (if present) via the active work packet (`.foundry/WORK_PACKET.md`).",
    "Exception (high priority): open feedback ledger items with `status: \"open\"` and `shouldImplement: true` are ALWAYS in scope for this pass, even if they are outside the BUILD_SPEC primary slice. Do NOT defer them as 'out of slice' when they are listed in build targets/iteration focus.",
    "",
    ...
    "   - Resolve MULTIPLE items per pass: target at least 2 feedback items (or all remaining if fewer than 2).",
    "   - Prefer clustered fixes: choose feedback that touches the same screen/module so one pass can close 2-4 items.",
    ...
    "    - If you resolve fewer than 2 feedback items in this pass while 2+ were open, document concrete blockers in `## Remaining Blockers` with the exact item IDs/summaries.",
    ...
    "- When BUILD_SPEC exists, implement one primary slice by default, BUT open `shouldImplement` feedback items remain in-scope and must be implemented in the same pass where feasible.",
