...
import { readLatestArtifact } from "@foundry/core";
import { stripBriefIdComment } from "@foundry/core/briefIntent";
export async function readDiffStatsForHead(repoPath) {
...
export async function readBuilderRemainingBlockers(repoPath) {
...
export function extractManualUnblocks(items) {
    const patterns = [
        /manual action/i,
        ...
        /environmental — not a code failure/i,
        /no booted simulator\/device/i,
        /boot an ios simulator/i,
        /connect a device\b/i,
        /qa_automation\.maestro\.required.*false/i,
        /install maestro/i,
        /maestro cli/i,
        /^maestro smoke did not run\b/i,
...
export function separateManualAndCodeItems(items) {
...
export function qaCodeBlockersAreOnlyMaestroSmokeFailed(qa) {
...
export function resolveCursorAutomationSettings(config, overrides) {
    ...
    return {
        ...
        builderFastModel: overrides?.builderFastModel ??
            ...
            envFast ??
            "auto",
        builderEconomyModel: overrides?.builderEconomyModel ??
...
export function qaNeedsPremiumCursorBuilder(qa) {
    if (!qa)
        return false;
    ...
    if (qa.testsRan === true && qa.testsPassed === false)
        return true;
    if (qaMaestroSmokeOnlyShipGreen(qa))
        return false;
 ...
 */
export function resolveEffectiveInnerLoopMax(baseMaxInner, qa) {
    if (!qaNeedsPremiumCursorBuilder(qa))
        return baseMaxInner;
...
const CORE_QA_CHECK_ORDER = ["test_suite", "lint", "typecheck", "maestro_smoke"];
/** One-line summary of the main automated QA steps (from last `independent_qa` artifact). */
export function summarizeCoreQaPlanForConsole(qa) {
 ...
 */
export function shouldUseNearReleaseEconomyBuilder(releaseStatus, pipelineQaRecommendation, pipelineQaCodeBlockerCount, feedbackImplementQueued) {
    if (pipelineQaRecommendation !== "ship" || pipelineQaCodeBlockerCount > 0)
        return false;
    if (feedbackImplementQueued > 0)
        return false;
    return releaseStatus === "awaiting_approval";
}
export function chooseBuilderModel(settings, briefCounts, pipelineQa, stalledIterations, innerLoopIndex, feedbackImplementQueued, releaseStatus, investorBelowTarget = false) {
    ...
    if (settings.useBuilderEconomyNearRelease &&
        !investorBelowTarget &&
        shouldUseNearReleaseEconomyBuilder(releaseStatus, pipelineQaRecommendation, pipelineQaCodeBlockerCount, feedbackImplementQueued)) {
        return {
            model: economy,
    ...
    const hasWholesaleSurface = briefCounts.total > 0 || pipelineQaCodeBlockerCount > 0 || feedbackImplementQueued > 0;
    if (hasWholesaleSurface) {
        return {
            model: primary,
            reason: "first inner pass — primary builder for wholesale work across the full work packet + CURSOR_BRIEF (read listed files)",
        };
    }
    return {
        model: fast,
...
export async function readStageJson(repoPath, manifest, stageName) {
 ...
 */
export function manifestStageRan(manifest, stageName) {
    if (!manifest)
        return false;
...
export async function logShipGateConsole(repoPath, manifest, cached) {
    const pipelineQa = cached?.pipelineQa ??
        (await readStageJson(repoPath, manifest, "independent_qa"));
    const release = cached?.release ?? (await readStageJson(repoPath, manifest, "release_agent"));
    ...
    if (bl.length === 0) {
        ...
        if (bl.length > 14)
            console.log(chalk.gray(`  │   … +${bl.length - 14} more (see independent_qa/output.json)`));
    }
    ...
    if (manual.length > 0) {
        ...
        if (manual.length > 8)
            console.log(chalk.gray(`  │   … +${manual.length - 8} more (see independent_qa/output.json)`));
    }
    ...
    if (screenshots.length > 0) {
        ...
        if (screenshots.length > 6)
            console.log(chalk.gray(`  │   … +${screenshots.length - 6} more (see independent_qa/output.json)`));
    }
    if (!release) {
    ...
    const stChalk = st === "awaiting_approval" || st === "auto_approved" || st === "approved"
        ? chalk.green
        : st === "blocked_by_qa"
            ? chalk.red
    ...
    console.log(chalk.gray(`  │ release_agent fields: qaRecommendation=${release.qaRecommendation ?? "?"} · qaScore=${release.qaScore ?? "?"} · easAvailable=${String(release.easAvailable ?? "?")}`));
    if (release.version)
        console.log(chalk.gray(`  │ app version: ${release.version}`));
    if (release.manualApprovalRequired) {
        console.log(chalk.yellow(`  │ manualApprovalRequired: true`));
        if (release.approvalFile)
            console.log(chalk.yellow(`  │ approvalFile: ${release.approvalFile}`));
    }
    ...
    if (release.notes?.length) {
        ...
        for (const n of release.notes)
            console.log(chalk.gray(`  │   - ${truncateForDisplay(n, 100)}`));
    }
    ...
    if (investorStage?.status === "skipped") {
        ...
        if (investorStage.skipCause === "convergence") {
            const cc = await readStageJson(repoPath, manifest, "convergence_contract");
            if (cc) {
                const open = (cc.openObjections ?? []).filter((o) => o.status === "open" || o.status === "regressed");
                if (open.length > 0) {
                    console.log(chalk.yellow(`  │   open objections (${open.length}):`));
                    open.slice(0, 5).forEach((o, i) => {
                        console.log(chalk.yellow(`  │     ${i + 1}. [${o.status}] ${truncateForDisplay(o.objection, 88)}`));
                        if (o.requiredEvidence) {
                            console.log(chalk.gray(`  │        evidence: ${truncateForDisplay(o.requiredEvidence, 88)}`));
                        }
                    });
                    if (open.length > 5)
                        console.log(chalk.gray(`  │     … +${open.length - 5} more`));
                }
        ...
        console.log(chalk.gray("  │   Pitch must reflect built product+repo. Run `foundry convergence status` and `foundry ledger list` to see what blocks the next pitch."));
        ...
        if (investorStage.skipCause === "directives_unaddressed") {
            console.log(chalk.yellow("  │   The pitch is deliberately held until BUILD_SPEC_LEDGER.addressedParents covers each prior directive. " +
                "Inspect .foundry/INVESTOR_PANEL_STATE.json (`lastDirectives`) and .foundry/BUILD_SPEC_LEDGER.json (`addressedParents`)."));
        }
        const nextCmd = investorStage.skipCause === "release_readiness"
...
export async function getPipelineSnapshotForConsole(repoPath, manifest, opts) {
    ...
    const qa = await readStageJson(repoPath, manifest, "independent_qa");
    const release = await readStageJson(repoPath, manifest, "release_agent");
...
function extractMarkdownSectionLines(md, headingLine) {
    ...
    for (let j = i + 1; j < lines.length; j++) {
        const line = lines[j];
        if (line.startsWith("## ") && line.trim() !== h)
            break;
        out.push(line);
    }
    return out;
}
function countBulletLines(sectionLines) {
    return sectionLines.filter((line) => /^\s*([-*]|\d+\.)\s/.test(line)).length;
}
function countTopLevelNumberedLines(sectionLines) {
    return sectionLines.filter((line) => /^\d+\.\s/.test(line.trim())).length;
}
function extractMarkdownSubsectionLines(md, headingLine) {
    const lines = md.split(/\r?\n/);
    const h = headingLine.trim();
    const i = lines.findIndex((l) => l.trim() === h);
    if (i < 0)
...
async function countTrackedBriefProgress(briefPath) {
    let raw = "";
    try {
...
export async function summarizeCursorBuilderReportMd(repoPath) {
    ...
    try {
        ...
        const feedbackResolved = countTopLevelNumberedLines(extractMarkdownSubsectionLines(raw, "### Feedback ledger items resolved:"));
        ...
        try {
            ...
            if (feedbackArtifact) {
                try {
                    const parsed = JSON.parse(feedbackArtifact);
...
export async function countCriticalBriefItems(briefPath) {
    const counts = {
        mustShip: 0,
        shouldShip: 0,
        unresolvedGaps: 0,
    ...
    for (const line of raw.split("\n")) {
        ...
        if (!trimmed.startsWith("- [ ]"))
            continue;
        if (section === "must" && !wizardSourcedMust)
            counts.mustShip++;
        if (section === "should")
            counts.shouldShip++;
        if (section === "gaps")
...
function briefSectionLabel(section) {
    switch (section) {
        case "must":
            return "Must ship";
        case "should":
...
export function truncateForDisplay(text, maxLen) {
 ...
 */
export async function sampleUncheckedBriefLines(briefPath, maxTotal, maxLineLen = 130) {
    let raw = "";
    try {
        raw = await readFile(briefPath, "utf8");
    }
...
export function formatIterationFocusMarkdown(params) {
    ...
    if (params.stabilize) {
        lines.push("> **Stabilize mode:** outer cycles skip upstream product/gap/monetization pipeline stages. Goal: `independent_qa` → **ship** with **zero blockers** (tests, lint, typecheck, Maestro), then full brief scope returns for release checklist items.", "");
    }
    if (params.openTrackedBriefSample && params.openTrackedBriefSample.length > 0) {
        ...
        for (const L of params.openTrackedBriefSample)
            lines.push(`- ${L}`);
        lines.push("", "_Also: `.foundry/OPEN_TRACKED_BRIEF.md` after each pipeline run._", "");
    }
    lines.push("## Active work packet counts", "", `- must=${params.briefCounts.mustShip}, should=${params.briefCounts.shouldShip}, gaps=${params.briefCounts.unresolvedGaps}, monetization=${params.briefCounts.monetization}, edge=${params.briefCounts.edgeFunctions}, runtime=${params.briefCounts.runtime}`, "", "## Pipeline QA (`independent_qa`) — before this builder pass", "", `- recommendation: \`${params.pipelineQa?.recommendation ?? "missing"}\``, `- score: ${params.pipelineQa?.score ?? "—"}`, "");
    if (params.pipelineQa?.blockers?.length) {
        lines.push("### Blockers (builder should drive these to zero)", "");
        for (const b of params.pipelineQa.blockers)
            lines.push(`- ${b}`);
        lines.push("");
    }
    ...
    if (params.pipelineQa?.warnings?.length) {
        lines.push("### Warnings", "");
        for (const w of params.pipelineQa.warnings)
            lines.push(`- ${w}`);
        lines.push("");
    }
    ...
    if (params.builderRemainingBlockers.length === 0) {
        ...
        for (const b of params.builderRemainingBlockers)
            lines.push(`- ${b}`);
        lines.push("");
    }
    ...
    if (params.briefSamples.length === 0) {
        ...
        for (const s of params.briefSamples)
            lines.push(`- ${s}`);
        lines.push("");
    }
    lines.push("## Next step", "");
    lines.push("Re-run the **Foundry pipeline** so `independent_qa` re-executes tests/lint/typecheck. Ship/release gates use **only** that stage — there is no separate Cursor QA pass.", "");
    return lines.join("\n");
}
...
function isGeneratedFoundryPath(path) {
    if (GENERATED_FOUNDRY_PATHS.has(path))
        return true;
 ...
 */
async function filterStageableGitPaths(repoPath, paths) {
    const stage = [];
    const skipped = [];
...
async function classifyPorcelainPaths(repoPath) {
...
function formatBuilderStatusHint(classified) {
...
async function ensureCursorBuilderBranch(repoPath, manifest) {
    const builder = await readStageJson(repoPath, manifest, "builder");
    ...
    const checkoutCmd = exists.exitCode === 0
    ...
    if (checkedOut.exitCode !== 0) {
        throw new Error(`Could not switch Cursor builder to branch '${targetBranch}'.\n${checkedOut.stderr || checkedOut.stdout}`.trim());
    }
}
function formatCursorBuilderCommitMessage(runId, files) {
 ...
 */
function looksLikeRemoteAheadPushRejection(text) {
    ...
    return (t.includes("non-fast-forward") ||
...
async function pushWithUpstreamRecovery(repoPath, branch) {
    ...
    const merge = await exec(`git merge ${shellQuote(`origin/${branch}`)} -m ${shellQuote("foundry: merge origin before push")} -X ours`, repoPath, 180_000);
    if (merge.exitCode === 0) {
        r = await push();
        if (r.exitCode === 0)
            return { ok: true, detail: "" };
...
async function autoCommitCursorBuilderChanges(repoPath, runId) {
    ...
    if (stage.length === 0) {
        throw new Error(`Cursor builder changes could not be staged — no valid paths after filtering git status.\n` +
            (skipped.length ? `Skipped (missing and not tracked): ${skipped.join(", ")}` : ""));
    }
    const files = stage;
    const add = await exec(`git add -A -- ${files.map((file) => shellQuote(file)).join(" ")}`, repoPath, 30_000);
    if (add.exitCode !== 0) {
    ...
    const commit = await exec(`git commit -m ${shellQuote(subject)} -m ${shellQuote(body)}`, repoPath, 60_000);
    if (commit.exitCode !== 0) {
        throw new Error(`Cursor builder left code changes uncommitted and the auto-commit failed.\n${commit.stderr || commit.stdout}`.trim());
    }
    ...
    if (headNames.exitCode === 0) {
        ...
        if (committed.length === 0 || missing.length === files.length) {
            throw new Error("Cursor builder commit verification failed: HEAD did not include expected non-generated product file paths.");
        }
    }
...
function transportWatchdogGraceMs() {
 ...
 */
export function isLikelyCursorTransportFailure(text) {
    if (!text)
        return false;
    return /reconnect-only output|\[transport-watchdog\]|Connection lost,?\s*reconnecting|network error.*reconnect/i.test(text);
}
function exec(command, cwd, timeoutMs, liveLogPath, appendLog) {
    return new Promise((resolve) => {
        ...
        const child = execFile(shell, ["-lc", shellBootstrapCommand(command)], {
            cwd,
            env: process.env,
            timeout: timeoutMs,
            maxBuffer: 16 * 1024 * 1024,
        }, (error, stdout, stderr) => {
            const extra = error && typeof error === "object" && "message" in error
            ...
            resolve({
                ...
                stderr: [typeof stderr === "string" ? stderr : "", extra, transportExtra]
                    .filter(Boolean)
                    .join("\n") || "",
            });
        });
        const noteTransportActivity = (chunk) => {
            const text = typeof chunk === "string" ? chunk : Buffer.isBuffer(chunk) ? chunk.toString("utf8") : String(chunk);
            if (transportFailurePattern.test(text)) {
                if (transportFailureGraceMs <= 0)
                    return;
                if (!reconnectTimer) {
...
function stagePath(repoPath, manifest, stageName, filename = "output.json") {
    const idx = manifest.stages.findIndex((s) => s.stage === stageName);
    const prefix = idx >= 0 ? String(idx).padStart(2, "0") : "99";
    return join(repoPath, ".foundry", "out", manifest.runId, `${prefix}_${stageName}`, filename);
}
function buildBuilderPrompt(repoPath, manifest, options) {
    const iterationFocusPath = options?.innerLoopIndex !== undefined
        ? join(repoPath, ".foundry", "automation", manifest.runId, `iteration-${options.innerLoopIndex}-focus.md`)
        : undefined;
    const base = [
    ...
    if (!options?.implementationRetry)
        return base;
    return [
        base,
        "",
...
async function runCursorAgent(repoPath, runId, command, model, prompt, timeoutMinutes, options) {
...
export async function runBuilderAgent(repoPath, manifest, settings, modelOverride, builderContext) {
    try {
    ...
    let result = await runCursorAgent(repoPath, manifest.runId, settings.command, chosenModel, buildBuilderPrompt(repoPath, manifest, { innerLoopIndex: innerIdx }), settings.timeoutMinutes);
    if (!result.ok)
        return result;
    let statusHint;
    ...
    try {
        ...
        result = await runCursorAgent(repoPath, manifest.runId, settings.command, chosenModel, buildBuilderPrompt(repoPath, manifest, { implementationRetry: true, innerLoopIndex: innerIdx }), settings.timeoutMinutes, { appendLog: true });
...
export async function preflightCursorCommand(command, cwd) {
    const shell = process.env.SHELL || "/bin/bash";
    ...
    if (availableResult.exitCode !== 0) {
        return {
            ok: false,
            detail: `Cursor command '${command}' could not be executed from the current shell. ` +
                `Set 'cursor_automation.command' in .foundry/project.yaml to the absolute binary path if needed.`,
        };
    }
    const authResult = await execFilePreflight(shell, ["-lc", shellBootstrapCommand(`${command} status`)], cwd);
    ...
    if (/Cannot find module '@anysphere\/file-service/i.test(combined)) {
        return {
            ok: false,
            detail: `Cursor command '${command}' failed to load a native dependency (@anysphere/file-service-*). ` +
    ...
    const positiveAuth = authResult.exitCode === 0 &&
    ...
    const explicitlyUnauthed = /authentication required|not logged in|login required|please (?:log ?in|sign ?in)|run [`'"]?cursor-agent login/i.test(stripped);
...
function cursorModelSkipsAvailabilityCheck(model) {
...
export async function preflightCursorModels(command, cwd, models) {
    const shell = process.env.SHELL || "/bin/bash";
    const result = await execFilePreflight(shell, ["-lc", shellBootstrapCommand(`${command} models`)], cwd);
    const combined = `${result.stdout}\n${result.stderr}`;
    if (result.exitCode !== 0) {
        return {
            ok: false,
            detail: `Could not list models from Cursor CLI. Run '${command} models' manually and verify access.`,
        };
    }
    ...
    if (missing.length > 0) {
        ...
        return {
            ok: false,
            detail: `Configured Cursor model(s) not in '${command} models' list:\n  - ${suggestions.join("\n  - ")}\n` +
...
function execFilePreflight(cmd, args, cwd) {
    return new Promise((resolve) => {
        execFile(cmd, args, { cwd, env: process.env, timeout: 15_000, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => {
            resolve({
                exitCode: error ? 1 : 0,
                stdout: typeof stdout === "string" ? stdout : "",
