...
function isEasEligibleConfig(config) {
...
function easCommandWithArgs(command, args) {
...
function parsePorcelainV1Path(line) {
...
function hasNonFoundryWorkingTreeChanges(porcelain) {
    for (const raw of porcelain.split("\n")) {
        const p = parsePorcelainV1Path(raw);
        if (!p)
            continue;
        if (p === ".foundry" || p.startsWith(".foundry/"))
            continue;
        return true;
    }
    return false;
}
function foundryWorkingTreePaths(porcelain) {
    const out = [];
    for (const raw of porcelain.split("\n")) {
        const p = parsePorcelainV1Path(raw);
        if (!p)
    ...
    return new Promise((resolve) => {
        ...
        execFile(shell, ["-lc", shellBootstrapCommand(command)], {
            ...
            resolve({
                ...
                stderr: [typeof stderr === "string" ? stderr : "", extra]
                    .filter(Boolean)
                    .join("\n") || "",
            });
        });
...
function walkForExpoBuildUrls(value, out) {
    if (value === null || value === undefined)
        return;
    if (typeof value === "string") {
        if (value.includes("expo.dev") && value.includes("/builds/")) {
            const m = value.match(/https:\/\/expo\.dev\/accounts\/[^/\s"'<>]+\/projects\/[^/\s"'<>]+\/builds\/[a-zA-Z0-9-]+/);
            if (m)
                out.add(m[0]);
        }
        return;
    ...
    if (Array.isArray(value)) {
        for (const x of value)
            walkForExpoBuildUrls(x, out);
        return;
    }
    if (typeof value === "object") {
        for (const v of Object.values(value))
            walkForExpoBuildUrls(v, out);
    }
}
function parseBuildInfo(stdout, stderr = "") {
    const combined = `${stdout}\n${stderr}`;
    const fromRegex = [
        ...combined.matchAll(/https:\/\/expo\.dev\/accounts\/[^/\s"'<>]+\/projects\/[^/\s"'<>]+\/builds\/[a-zA-Z0-9-]+/g),
    ].map((m) => m[0]);
        ...
        try {
            ...
            for (const item of items) {
                if (!item || typeof item !== "object")
                    continue;
                const obj = item;
                if (!buildId && typeof obj.id === "string")
                    buildId = obj.id;
                const w = typeof obj.webUrl === "string"
                    ? obj.webUrl
    ...
    const urls = [...urlSet];
    if (urls.length > 0)
        buildUrl = urls[urls.length - 1];
    return { buildId, buildUrl };
}
const EAS_BUILD_QR_PNG = "eas-build-qr.png";
async function enrichEasResultWithQr(result, releaseDir) {
    if (!result.buildUrl)
        return result;
    try {
...
async function finishEasInstallDocs(repoPath, manifest, settings, result) {
...
async function persistPromotionLog(repoPath, manifest, result, stdout = "", stderr = "") {
    const releaseDir = join(repoPath, ".foundry", "releases", manifest.runId);
...
async function autoCommitAndPushFoundryArtifacts(repoPath, branchName, runId) {
    let stdout = "";
    let stderr = "";
    ...
    if (add.exitCode !== 0)
        return { ok: false, stdout, stderr };
    const commit = await exec(`git commit -m ${shellQuote(`foundry: update run artifacts ${runId}`)}`, repoPath, 60_000);
...
export async function promoteApprovedBranch(repoPath, manifest, builder) {
    const builderBranch = builder?.branchName;
    const targetBranch = "main";
    if (!builderBranch) {
        return persistPromotionLog(repoPath, manifest, { status: "skipped", detail: "Builder branch metadata not available; skipping branch promotion." });
    }
    const remote = await exec("git remote get-url origin", repoPath, 15_000);
    if (remote.exitCode !== 0) {
        return persistPromotionLog(repoPath, manifest, {
            status: "failed",
    ...
    if (hasNonFoundryWorkingTreeChanges(status.stdout)) {
        return persistPromotionLog(repoPath, manifest, {
            status: "failed",
            detail: "Working tree has changes outside `.foundry/`; commit or stash them, then re-run promotion (or merge/push manually). Changes only under `.foundry/` (e.g. approvals, run output) no longer block automatic promotion.",
            baseBranch: targetBranch,
            builderBranch,
        }, status.stdout, status.stderr);
    }
    ...
    if (currentBranch.exitCode !== 0) {
        return persistPromotionLog(repoPath, manifest, {
            ...
            baseBranch: targetBranch,
            builderBranch,
        }, currentBranch.stdout, currentBranch.stderr);
    }
    ...
    if (builderExists.exitCode !== 0) {
        return persistPromotionLog(repoPath, manifest, {
            ...
            baseBranch: targetBranch,
            builderBranch,
        }, builderExists.stdout, builderExists.stderr);
    }
    ...
    return persistPromotionLog(repoPath, manifest, {
        status: "promoted",
        detail: builderBranch === targetBranch
...
export function resolveEasBuildSettings(config) {
...
async function writeLatestInstallFile(repoPath, manifest, settings, result) {
    const lines = [
        "# Latest Install",
        "",
        `- Run: \`${manifest.runId}\``,
...
export async function maybeRunApprovedEasBuild(repoPath, config, manifest, releaseStatus, opts) {
    const settings = resolveEasBuildSettings(config);
    if (!isEasEligibleConfig(config)) {
        const result = { status: "skipped", detail: "EAS build skipped: project repo_type is not mobile/Expo." };
        return finishEasInstallDocs(repoPath, manifest, settings, result);
    }
    if (!settings.buildOnApproval && !opts?.force) {
        const result = { status: "skipped", detail: "EAS build on approval is disabled in .foundry/project.yaml." };
        return finishEasInstallDocs(repoPath, manifest, settings, result);
    }
    if (releaseStatus !== "approved" && releaseStatus !== "auto_approved") {
        const result = { status: "skipped", detail: "Release is not approved yet." };
        return finishEasInstallDocs(repoPath, manifest, settings, result);
    }
    const releaseDir = join(repoPath, ".foundry", "releases", manifest.runId);
    ...
    await mkdir(releaseDir, { recursive: true });
    const persist = async (result, stdout = "", stderr = "") => {
        const log = [
            `status: ${result.status}`,
            `detail: ${result.detail}`,
            "",
    ...
    if (availability.exitCode !== 0) {
        ...
        }, availability.stdout, availability.stderr);
    ...
    const remoteDetails = await fetchExpoBuildDetails(settings.command, buildInfo.buildId, repoPath, logUrlsFromStdout);
    if (remoteDetails) {
...
const EXPO_BUILD_URL_RE = /https:\/\/expo\.dev\/accounts\/[^/\s"'<>]+\/projects\/[^/\s"'<>]+\/builds\/([a-zA-Z0-9-]{8,})/g;
const MAX_EXPO_BACKFILL_RUN_DIRS = 6;
/** Best-effort: pull the JSON array/object that EAS prints when invoked with `--json`. */
function extractEasBuildJson(text) {
    try {
 ...
 */
export function extractExpoBuildLogUrls(text) {
...
function buildIdFromJson(parsed, fallbackText) {
    ...
    if (m) {
        ...
        if (tail)
            return tail.split(/[^a-zA-Z0-9-]/)[0];
    }
    return undefined;
}
function normalizeExpoBuildViewPayload(input, buildId) {
    ...
    const candidate = Array.isArray(root)
        ...
        : root.data &&
            ...
            root.data.builds &&
            typeof root.data.builds === "object" &&
            root.data.builds.byId
            ? root.data.builds.byId
            : root;
    ...
    if (typeof build.id === "string")
        ...
        out.id = buildId;
    if (typeof build.status === "string")
        out.status = build.status;
    if (typeof build.platform === "string")
        out.platform = build.platform;
    if (typeof build.appVersion === "string")
        out.appVersion = build.appVersion;
    if (typeof build.sdkVersion === "string")
        out.sdkVersion = build.sdkVersion;
    if (build.error && typeof build.error === "object")
        out.error = build.error;
    const artifacts = build.artifacts;
    if (artifacts && typeof artifacts === "object")
        out.artifacts = artifacts;
    if (Array.isArray(build.logFiles) || typeof build.logFiles === "string")
        out.logFiles = build.logFiles;
    if (typeof build.logsUrl === "string")
        out.logsUrl = build.logsUrl;
    if (typeof build.logs === "string")
        out.logs = build.logs;
 ...
 */
export async function backfillExpoBuildViews(repoPath, config) {
    const debug = process.env.FOUNDRY_DEBUG_EXPO_BACKFILL === "1";
    const log = (msg) => {
        if (debug)
            console.error(`[expo-backfill] ${msg}`);
    };
    try {
        const settings = resolveEasBuildSettings(config);
        ...
        for (const runId of runDirs) {
            ...
            for (const name of entries) {
                if (!/\.(log|txt|json)$/i.test(name))
                    continue;
                if (name === "expo-build-view.json" || name === "expo-build-logs.txt")
                    continue;
                ...
                log(`${runId}/${name}: got view (${remote.raw.length}b) logs=${remote.remoteLogs?.length ?? 0}b`);
                if (!hasView)
                    await writeFile(canonicalView, remote.raw, "utf8");
                if (!hasLogs && remote.remoteLogs)
                    await writeFile(canonicalLogs, remote.remoteLogs, "utf8");
                break;
            }
        }
    }
...
let cachedExpoSessionSecret;
/** Read Expo's CLI session secret from `~/.expo/state.json`. Memoized. */
async function readExpoSessionSecret() {
    if (cachedExpoSessionSecret !== undefined)
 ...
 */
async function fetchExpoBuildViaGraphQL(buildId) {
    const session = await readExpoSessionSecret();
    try {
        const query = "query GetBuild($buildId: ID!) { builds { byId(buildId: $buildId) { id status error { errorCode message docsUrl } logFiles artifacts { xcodeBuildLogsUrl buildUrl applicationArchiveUrl } } } }";
        ...
        if (!b)
            return null;
        const logFiles = [];
        if (Array.isArray(b.logFiles))
            for (const u of b.logFiles)
                if (typeof u === "string")
                    logFiles.push(u);
                    ...
                    try {
                        ...
                        if (/^https?:\/\//i.test(b.logFiles))
                            logFiles.push(b.logFiles);
                    }
                }
        const xcodeLogsUrl = typeof b.artifacts?.xcodeBuildLogsUrl === "string" ? b.artifacts.xcodeBuildLogsUrl : null;
        return {
            logFiles,
            xcodeLogsUrl,
...
async function tryFetchExpoBuildPageLogs(buildId) {
...
async function fetchUrlText(url, timeoutMs = 45_000, maxBytes = 4 * 1024 * 1024) {
    try {
        const controller = new AbortController();
...
function summarizeExpoBuildJson(buildId, parsed) {
    if (!parsed)
        return [`buildId: ${buildId}`, "(could not parse build JSON)"];
    const lines = [`buildId: ${buildId}`];
    const push = (label, v) => {
    ...
    if (urls.length)
        lines.push(`logUrlCount: ${urls.length}`);
    return lines;
}
/**
 ...
 */
async function fetchExpoBuildDetails(easCommand, buildId, cwd, candidateUrls = []) {
    if (!buildId)
        return null;
    const tryFetchAll = async (urls) => {
        const out = [];
    ...
    if (graphql) {
        ...
        if (!remoteLogs) {
            ...
            if (freshUrls.length)
                remoteLogs = await tryFetchAll(freshUrls);
        }
    ...
    summaryLines.push(remoteLogs ? `remoteLogsBytes: ${remoteLogs.length}` : "remoteLogsBytes: 0 (fetch failed)");
    if (!graphql && !(await readExpoSessionSecret())) {
        summaryLines.push("note: no ~/.expo/state.json sessionSecret found; some log URLs may 403. Run `eas login` to authenticate.");
    }
    const raw = JSON.stringify(normalizeExpoBuildViewPayload(summaryParsed, buildId) ?? {
        id: buildId,
        status: graphql?.status ?? "UNKNOWN",
        error: graphql?.error ?? {
    ...
    if (!summaryParsed && !remoteLogs)
        return null;
    return { raw, summary: summaryLines.join("\n"), remoteLogs: remoteLogs || undefined };
}
export async function maybeSubmitLatestToTestFlight(repoPath, config, manifest, releaseStatus) {
    const settings = resolveEasBuildSettings(config);
    if (!isEasEligibleConfig(config)) {
