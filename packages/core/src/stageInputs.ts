...
export type InvestorRefinementContext = z.infer<typeof InvestorRefinementContextSchema>;
...
export type StageInputComposition = z.infer<typeof StageInputCompositionSchema>;

/**
 * Context needed to build composed stage inputs: prior stage outputs keyed by
 * pipeline stage name (e.g. `repo_inventory`).
 */
export interface StageInputContext extends RunContext {
  priorOutputs: Record<string, unknown>;
  /** Set by `runPipeline` when re-running stages after `investor_panel` feedback. */
  investorRefinementLoop?: InvestorRefinementContext;
}
...
export async function getStageInput(
  _stageName: string,
  ctx: StageInputContext,
): Promise<StageInputComposition> {
  const o = ctx.priorOutputs;

  async function readLatestStageOutput(stageName: string): Promise<unknown> {
