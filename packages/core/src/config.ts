...
const BuilderDirectiveSchema = z.preprocess(
  (value) => {
    if (typeof value === "string") return { description: value, files: [] };
    return value;
  },
  z.object({
...
function parseMetricTarget(value: unknown): number {
...
export type MetricsYaml = z.infer<typeof MetricsYamlSchema>;
export type GatesYaml = z.infer<typeof GatesYamlSchema>;
...
const REQUIRED_CONFIG_FILES = ["project.yaml", "metrics.yaml", "gates.yaml"] as const;

export async function loadFoundryConfig(repoPath: string): Promise<FoundryConfig> {
  const foundryDir = join(repoPath, ".foundry");
  const missingFiles = (
    await Promise.all(
      REQUIRED_CONFIG_FILES.map(async (name) => {
        try {
  ...
  ).filter((name): name is (typeof REQUIRED_CONFIG_FILES)[number] => Boolean(name));

  if (missingFiles.length > 0) {
    throw new Error(
      `Missing Foundry config file${missingFiles.length === 1 ? "" : "s"}: ${missingFiles.join(", ")} in ${foundryDir}. Run \`foundry init --repo ${repoPath}\` first.`,
    );
  }
