...
function labelForBriefSection(section) {
    switch (section) {
        case "must":
            return "Must ship";
        case "should":
...
async function writeOpenTrackedBriefMirror(repoPath, summary) {
    ...
    if (summary.openTrackedLines.length === 0) {
        ...
        if (summary.openTrackedItems > summary.openTrackedLines.length) {
            lines.push(`_Showing first ${summary.openTrackedLines.length} of ${summary.openTrackedItems} — open \`${summary.briefPath}\` for the full list._`, "");
        }
        for (const L of summary.openTrackedLines)
            lines.push(`- ${L}`);
        lines.push("");
    }
...
function isReleaseAgentGate(gate) {
    return gate.startsWith("release_agent.");
}
function releaseAgentGatesFromConfig(requireHumanApproval) {
    return requireHumanApproval.filter(isReleaseAgentGate);
}
async function readVersionFromConfigFile(abs) {
...
async function readExpoVersion(repoPath, input) {
    const candidates = [];
    ...
    candidates.push(join(repoPath, "app.json"), join(repoPath, "app.config.json"), join(repoPath, "app.config.ts"), join(repoPath, "app.config.js"), join(repoPath, "app.config.mjs"), join(repoPath, "app.config.cjs"));
    const seen = new Set();
...
async function checkEasAvailable(repoPath) {
    const r = await sh("eas --version", repoPath, 15_000);
    return r.exitCode === 0;
}
async function computeApprovalToken(repoPath, builder, qa, version) {
...
function approvalFileRelative(approvalToken) {
    return `.foundry/approvals/${approvalToken}.approved`;
}
function humanizeReleaseItem(text) {
...
function buildReleaseNotes(projectName, builder) {
    const candidates = [
    ...
    if (bullets.length === 0) {
        bullets.push(`Performance and polish improvements across ${projectName}.`, "Minor reliability improvements and bug fixes.", "General UX refinements for a smoother experience.");
    }
    return {
...
function buildManualReleaseSteps(repoPath, approvalRel, easAvailable, version) {
    return [
...
async function writeApprovalRequiredMd(repoPath, approvalToken, runId, projectName, builder, qa, version, easPlan, manualSteps, releaseNotes) {
    ...
    if (builder) {
        ...
        if (builder.plan.gapsAddressed.length > 0) {
            ...
            for (const g of builder.plan.gapsAddressed)
                lines.push(`- ${g}`);
            lines.push("");
        }
        if (builder.commit) {
    ...
    lines.push("- Optional Cursor session notes: `.foundry/CURSOR_BUILDER_REPORT.md` and `.foundry/CURSOR_QA_REPORT.md` (ship decision is from `independent_qa` only)");
    if (easPlan?.enabled) {
    ...
    if (releaseNotes) {
        ...
        for (const bullet of releaseNotes.bullets)
            lines.push(`- ${bullet}`);
        lines.push("");
    }
    ...
    // Approve
    lines.push("## Approve", "", "```bash", `mkdir -p .foundry/approvals && touch ${rel}`, "```", "", "This approval is tied to the current release snapshot, so it remains valid across loop cycles until the code changes.", "");
    await writeFile(join(repoPath, ".foundry", "APPROVAL_REQUIRED.md"), lines.join("\n"), "utf8");
}
function buildReadmeMarkdown(projectName, out) {
    const statusLine = out.status === "approved"
...
export const releaseAgentStage = {
    name: "release_agent",
    description: "Prepare a release after independent QA: checklist, manual approval gate, EAS/version checks, and repo markers.",
    ...
    async run(ctx, input) {
        ...
        const qa = qaParsed.success ? qaParsed.data : undefined;
        if (!qaParsed.success) {
        ...
        if (!releasable) {
            if (!qaCleanForShip) {
                ...
                if (qa?.recommendation !== "ship") {
                    notes.push("independent_qa did not recommend ship — fix reported issues and re-run the pipeline. Optional Cursor agents may help implement fixes.");
                }
                else if (qaBlockers.length > 0) {
                    notes.push(`independent_qa recommends ship but lists ${qaBlockers.length} blocker(s) — clear them (e.g. Maestro, failing tests) before release.`);
                }
                if (builder && !builderReady) {
                    notes.push(`Builder status is "${builder.status}" — Foundry will not mark this release ready until the repo is clean enough for builder work or the builder completes successfully.`);
                }
                if (!briefReady) {
                    notes.push(`CURSOR_BRIEF has ${briefSummary.openTrackedItems} tracked open item(s) — finish after QA is ship with zero blockers, then close items in ${briefSummary.briefPath} for release.`);
                }
            }
            else {
                status = "blocked_pre_release";
                notes.push("QA is clean (ship + no blockers), but pre-release gates are still blocked (tracked brief, builder readiness, or checklist). Finish those before approval.");
                if (builder && !builderReady) {
                    notes.push(`Builder status is "${builder.status}" — Foundry will not mark this release ready until the repo is clean enough for builder work or the builder completes successfully.`);
                }
                if (!briefReady) {
                    notes.push(`CURSOR_BRIEF still has ${briefSummary.openTrackedItems} tracked open item(s) — complete/check off tracked items in ${briefSummary.briefPath} before release approval.`);
                }
            }
        }
