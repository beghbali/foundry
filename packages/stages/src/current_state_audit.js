...
async function readSafe(abs, maxBytes = MAX_FILE_BYTES) {
    try {
...
async function collectFiles(root, extensions, maxDepth = 6, maxFiles = 500) {
    const result = [];
...
async function detectApp(repoPath, input) {
    ...
    if (!mobileRoot && !isWebDataPlatform) {
        if (await isDir(join(repoPath, "app")))
            mobileRoot = ".";
        else if (await isDir(join(repoPath, "apps", "mobile")))
            mobileRoot = "apps/mobile";
    }
    let webRoot;
    for (const candidate of ["apps/web", "web", "."]) {
        const abs = join(repoPath, candidate === "." ? "" : candidate);
        if ((await isDir(join(abs, "app"))) ||
    ...
    if (!backendRoot && (await isDir(join(repoPath, "supabase"))))
    ...
    if (hasTs && hasJs)
        ...
        language = "js";
    let packageManager;
    if (await readSafe(join(repoPath, "pnpm-lock.yaml")))
        packageManager = "pnpm";
    else if (await readSafe(join(repoPath, "yarn.lock")))
        packageManager = "yarn";
    else if (await readSafe(join(repoPath, "package-lock.json")))
        packageManager = "npm";
    return { mobileRoot, webRoot, backendRoot, dataRoots, cloudRoots, language, packageManager };
}
async function scanNavigation(repoPath, mobileRoot) {
    ...
    for (const abs of codeFiles) {
        ...
        if (isLayout) {
            let kind = "unknown";
            if (/\bStack\b/.test(content) || /createNativeStackNavigator/.test(content))
                kind = "stack";
            if (/\bTabs\b/.test(content) || /createBottomTabNavigator/.test(content))
                kind = "tabs";
...
async function scanWebNavigation(repoPath, webRoot) {
    ...
    for (const abs of codeFiles) {
        ...
        const relFromRepo = toPosix(relative(repoPath, abs));
        if (!/(^|\/)(app|pages|routes|src\/app|src\/pages|src\/routes)\//i.test(relFromRoot) &&
            !/(^|\/)(components|features)\//i.test(relFromRoot)) {
            continue;
        }
        const base = basename(abs);
        if (base.startsWith("_") && !/_layout|layout/i.test(base))
            continue;
        const content = await readSafe(abs);
        if (!content)
            continue;
        ...
        const isRoutable = /(^|\/)(app|pages|routes|src\/app|src\/pages|src\/routes)\//i.test(relFromRoot) &&
            !/\/(layout|loading|error|not-found)\.(tsx?|jsx?)$/i.test(relFromRoot);
        if (isRoutable)
            routes.push({ name: routeName, file: relFromRepo });
        const looksLikeScreen = isRoutable ||
            /\b(page|screen|dashboard|wizard|form|table|ledger|connector|agreement|policy|evidence|overlap)\b/i.test(relFromRepo) ||
...
function webFileToRouteName(relFromRoot) {
    return "/" + relFromRoot
...
function guessWebScreenPurpose(routeName, relFile, content) {
    const lower = `${routeName} ${relFile} ${content.slice(0, 1000)}`.toLowerCase();
    if (/connector|snowflake|redshift|databricks|bigquery|warehouse/.test(lower))
        return "Warehouse connector setup and verification";
    if (/agreement|contract|legal/.test(lower))
        return "Agreement generation and review";
    if (/policy|approval|approve/.test(lower))
        return "Policy approval and governance";
    if (/overlap|audience|cohort|segment/.test(lower))
        return "Audience overlap run workflow";
    if (/evidence|proof|receipt|pack/.test(lower))
        return "Evidence pack and proof review";
    if (/ledger|audit|history|runs/.test(lower))
        return "Trust Ledger / audit history";
    if (/org|tenant|partner|invite/.test(lower))
        return "Organization and partner management";
    if (/dashboard|home/.test(lower))
        return "Dashboard / workspace overview";
    if (/settings|admin|security/.test(lower))
        return "Admin, security, or configuration";
    return "Web application surface";
}
function fileToRouteName(relFromApp) {
    return "/" + relFromApp
        .replace(/\.(tsx?|jsx?|mjs|cjs)$/, "")
        .replace(/\/index$/, "")
        .replace(/^index$/, "");
}
function guessScreenPurpose(routeName, content) {
    const lower = routeName.toLowerCase();
    if (/auth/.test(lower))
        return "Authentication / sign-in flow";
    if (/onboarding/.test(lower))
        return "Onboarding step";
    if (/settings/.test(lower))
        return "User settings and preferences";
    if (/intro/.test(lower))
        return "Introduction or welcome screen";
    if (/pro/.test(lower) || /paywall/.test(lower))
        return "Pro upgrade / paywall";
    if (/memory/.test(lower))
        return "Garden memory / history view";
    if (/year/.test(lower))
        return "Yearly overview or calendar";
    if (/setup|garden/.test(lower))
        return "Garden setup and management";
    if (/observe/.test(lower))
        return "Observation / diagnosis input";
    if (/decision/.test(lower))
        return "Decision detail / action window";
    if (/plant/.test(lower))
        return "Plant detail view";
    if (/zone/.test(lower))
        return "Zone detail / edit";
    if (lower === "/" || lower === "")
        return "Entry / routing guard";
    if (/index/.test(lower) || /this.?week/.test(lower))
        return "Home / weekly action plan";
...
function extractKeyActions(content) {
    ...
    if (/supabase\s*\.\s*from\s*\(/.test(content))
        actions.push("Supabase DB query");
    if (/supabase\s*\.\s*auth\s*\./.test(content))
        actions.push("Supabase auth call");
    if (/supabase\s*\.\s*functions\s*\.invoke/.test(content))
        actions.push("Edge function invocation");
    if (/fetch\s*\(/.test(content) && !/deno/i.test(content))
        actions.push("HTTP fetch");
    if (/Alert\.alert/.test(content))
        actions.push("User alert dialog");
    if (/useEffect/.test(content))
        actions.push("Side effect on mount/change");
    if (/RefreshControl/.test(content))
        actions.push("Pull-to-refresh");
    if (/Image|<Image/.test(content))
        actions.push("Image display");
    if (/TextInput|<TextInput|<RNTextInput/.test(content))
        actions.push("Text input field");
    if (/Modal/.test(content))
        actions.push("Modal dialog");
    return actions.slice(0, 10);
}
async function scanApiSurface(repoPath, backendRoot) {
    const edgeFunctions = [];
    const endpoints = [];
    if (!backendRoot)
...
async function scanDataModel(repoPath, backendRoot, dataRoots = []) {
    const migrations = [];
    const allTables = new Set();
    const roots = new Set(dataRoots);
    if (backendRoot)
        roots.add(`${backendRoot}/migrations`);
...
function inferFlows(screens, routes, repoPath) {
    const flows = [];
    ...
    const weeklyScreen = screens.find((s) => s.file.includes("(tabs)/index") || s.purposeGuess.includes("weekly"));
    if (weeklyScreen) {
        ...
        if (observeScreen)
            involved.push(observeScreen.file);
        if (setupScreen)
            involved.push(setupScreen.file);
        flows.push({
...
async function fileContains(abs, pattern) {
    const content = await readSafe(abs);
    if (!content)
        return false;
    return typeof pattern === "string" ? content.includes(pattern) : pattern.test(content);
}
async function identifyGaps(repoPath, screens, api, data, flows, det) {
    const gaps = [];
    ...
    const hasErrorBoundary = screens.some((s) => /error/i.test(s.file)) ||
    ...
    const hasOfflineHandling = screens.some((s) => s.keyActions.some((a) => /offline|netinfo|network/i.test(a))) ||
    ...
    if (api.edgeFunctions.length > 0) {
        ...
        let hasRateLimiting = !!backendBase &&
    ...
    let hasInputValidation = screens.some((s) => s.keyActions.includes("Text input field") && s.keyActions.some((a) => /validat/i.test(a)));
    ...
    if (tablesWithRLS.length > 0 && !data.migrations.some((m) => m.tablesMentioned.length > 0)) {
        gaps.push({
            area: "data",
    ...
    if (deepLinkScreens.length > 0) {
        ...
        for (const s of deepLinkScreens) {
            ...
            const hasGuardByCode = await fileContains(join(repoPath, s.file), /\[foundry:auth-guard\]|useAuthStore\(\)|replace\(\s*['"]\/auth['"]\s*\)/);
            if (!hasGuardByMetadata && !hasGuardByCode)
                missingGuard.push(s);
        }
...
function buildReadme(output, projectName) {
    const lines = [
        ...
        "| Screen | File | Purpose |",
        "| --- | --- | --- |",
        ...output.screens.map((s) => `| ${s.name} | \`${s.file}\` | ${s.purposeGuess} |`),
        "",
        "## Edge Functions",
        "",
    ];
...
// ---------- stage ----------
export const currentStateAuditStage = {
    ...
    async run(ctx, input) {
        ...
        await writeStageMarkdown(ctx, "current_state_audit", "README.md", buildReadme(validated, input.config.project.project_name));
        return validated;
    },
};
