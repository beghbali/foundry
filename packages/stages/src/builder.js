...
import { annotateBriefLineWithId, briefIntentToken, mintBriefItemId, parseBriefIdComment, stripBriefIdComment, } from "@foundry/core/briefIntent";
import { GrandWizardOutputSchema, primaryBuildSpecSlice, readBuildSpecLedger, } from "@foundry/core/buildSpec";
import { StageInputCompositionSchema, } from "@foundry/core/stageInputs";
import { z } from "zod";
import { runQualityChecks, tidyCode } from "./_shared/cleanup.js";
import { computeSatisfiedEdgeRateLimitPaths, computeSatisfiedPaywallGates, resolveAnalytics, resolveAuthGuards, resolveErrorBoundary, resolveInputValidation, resolveOfflineHandler, resolvePaywallGates, resolveRateLimiting, wireAnalyticsIntoScreens, wirePaywallIntoScreens, wireRateLimitIntoEdgeFunctions, } from "./_shared/codegen.js";
...
function classifyGaps(gaps) {
    return gaps.map((g) => {
        ...
        if (/error\s*boundary/i.test(desc) || (g.area === "reliability" && /crash|unhandled/i.test(desc))) {
            return { gap: g.description, area: g.area, resolver: "error_boundary", likelyFiles: g.likelyFiles };
        }
        if (/offline|network\s*status|netinfo/i.test(desc)) {
            return { gap: g.description, area: g.area, resolver: "offline_handler", likelyFiles: g.likelyFiles };
        }
        if (/rate\s*limit/i.test(desc)) {
            return { gap: g.description, area: g.area, resolver: "rate_limit_wiring", likelyFiles: g.likelyFiles };
        }
        if (/auth\s*guard|bypass\s*login|without\s*auth|deep\s*link.*bypass/i.test(desc)) {
            return { gap: g.description, area: g.area, resolver: "auth_guard", likelyFiles: g.likelyFiles };
        }
        if (/validation|submit\s*bad\s*data|client.side\s*validation/i.test(desc)) {
            return { gap: g.description, area: g.area, resolver: "input_validation", likelyFiles: g.likelyFiles };
        }
        return { gap: g.description, area: g.area, resolver: "none", likelyFiles: g.likelyFiles };
    });
}
...
function guessFeedbackFiles(summary, screens, mobileRoot) {
    const lower = summary.toLowerCase();
    const out = new Set();
    const root = mobileRoot === "." ? "" : `${mobileRoot}/`;
    const allScreens = screens ?? [];
    const addMatchingScreens = (pattern) => {
...
function analyzeFeedback(feedback, screens, mobileRoot) {
    if (!feedback)
        return [];
    const actions = [];
    const items = (feedback.items ?? []).filter((item) => (item.status ?? "open") === "open" && (item.shouldImplement ?? item.repoActionable ?? true));
    const suggestions = feedback.suggestions ?? [];
    for (const item of items) {
        const lower = item.summary.toLowerCase();
        let relatedResolver = null;
        if (/app init|startup|boot|splash|launch|crash|error|broke|errorboundary/i.test(lower))
            relatedResolver = "error_boundary";
        if (/offline|network|connect/i.test(lower))
            relatedResolver = "offline_handler";
        if (/slow|timeout|rate/i.test(lower))
            relatedResolver = "rate_limiting";
        if (/pay|price|upgrade|premium/i.test(lower))
            relatedResolver = "paywall";
        if (/auth|login|sign in|session/i.test(lower) && relatedResolver === null)
            relatedResolver = "auth_guard";
...
function normalizeDirectiveKey(input) {
    return input.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim().slice(0, 160);
}
function inferDirectiveDomain(summary) {
    const lower = summary.toLowerCase();
    if (/\b(llm|prompt|model|agent|ai)\b/.test(lower))
        return "llm";
    if (/\b(auth|login|session|token|oauth)\b/.test(lower))
        return "auth";
    if (/\b(payment|paywall|pricing|subscription|entitlement|purchase)\b/.test(lower))
        return "monetization";
    if (/\b(scan|camera|barcode|ocr|image)\b/.test(lower))
        return "scanning";
    if (/\b(search|ranking|recommend)\b/.test(lower))
        return "search";
    if (/\b(notif|reminder|push)\b/.test(lower))
        return "notifications";
    if (/\b(profile|account|settings)\b/.test(lower))
        return "profile";
    if (/\b(sync|offline|network|cache)\b/.test(lower))
        return "sync";
    if (/\b(analytics|event|funnel|telemetry)\b/.test(lower))
        return "analytics";
    if (/\b(edge function|supabase|backend|api)\b/.test(lower))
        return "backend";
    return "product";
}
function resolverFromText(summary) {
    const lower = summary.toLowerCase();
    if (/error\s*boundary|crash|unhandled/.test(lower))
        return "error_boundary";
    if (/offline|network\s*status|netinfo/.test(lower))
        return "offline_handler";
    if (/rate\s*limit|throttle/.test(lower))
        return "rate_limit_wiring";
    if (/auth\s*guard|bypass\s*login|without\s*auth|deep\s*link.*bypass|session/.test(lower))
        return "auth_guard";
    if (/validation|submit\s*bad\s*data|invalid input/.test(lower))
        return "input_validation";
    if (/pay|price|upgrade|premium|subscription|entitlement/.test(lower))
        return "paywall";
    if (/analytics|event|funnel|track/.test(lower))
        return "analytics";
    return null;
}
function isCoreFocusDirective(text) {
...
async function loadAdaptiveResolverMemory(repoPath) {
...
async function saveAdaptiveResolverMemory(repoPath, memory) {
    const abs = join(repoPath, ADAPTIVE_RESOLVER_MEMORY_REL);
    await mkdir(join(repoPath, ".foundry"), { recursive: true });
    await writeFile(abs, JSON.stringify(memory, null, 2) + "\n", "utf8");
}
function buildAdaptiveResolverMemoryMd(memory) {
    const lines = [
...
function mergeLikelyFiles(current, incoming) {
    return [...new Set([...current, ...incoming].filter(Boolean))].slice(0, 24);
}
async function upsertAdaptiveDirectives(repoPath, runId, inputs) {
    const memory = await loadAdaptiveResolverMemory(repoPath);
    const byKey = new Map(memory.directives.map((d) => [d.key, d]));
...
function buildPlan(gapActions, feedbackActions, adaptiveDirectives, hasMonetization, monetizationNeedsResolverPass, hasEdgeFunctions, edgeNeedsResolverPass, isNode, opts) {
    const goals = [];
    const filesToTouch = [];
    const gapsAddressed = [];
    const feedbackAddressed = [];
    const risksNoted = [];
    const resolversToRun = new Set();
    const goalSeen = new Set();
    const riskSeen = new Set();
    const pushGoal = (goal) => {
        if (goalSeen.has(goal))
            return;
        goalSeen.add(goal);
        goals.push(goal);
    };
    const pushRisk = (risk) => {
        if (riskSeen.has(risk))
            return;
        riskSeen.add(risk);
    ...
    for (const fa of feedbackActions) {
        ...
        if (fa.relatedResolver) {
            if (!allowEdgeInfra && isEdgeInfrastructureResolver(fa.relatedResolver)) {
                pushRisk(`Deferred edge resolver (core product brief open): ${fa.summary.slice(0, 100)}`);
            }
            else if (!allowMonetInfra && isMonetizationInfrastructureResolver(fa.relatedResolver)) {
    ...
    for (const directive of adaptiveDirectives) {
        ...
        if (inferred) {
            const allowSecondary = !coreFocusMode ||
    ...
    const investorWantsMonetizationPass = investorDirectives.some((d) => directiveAllowsSecondaryWork(d));
    if (hasMonetization &&
...
// ---------- safe infrastructure ----------
async function applySafeInfrastructure(repoPath, isNode, logger) {
    const actions = [];
    const commandsRun = [];
...
// ---------- implementation plan ----------
function generateImplementationPlan(plan, allActions, projectName, baseBranch, baseSha) {
    const lines = [
    ...
    if (created.length > 0) {
        ...
        for (const a of created)
            lines.push(`- \`${a.file}\` — ${a.description}`);
        lines.push("");
    }
...
// ---------- build summary ----------
function generateSummary(output, projectName, baseBranch, baseSha) {
    const lines = [
    ...
    if (output.plan.gapsAddressed.length > 0) {
        ...
        for (const g of output.plan.gapsAddressed)
            lines.push(`- ${g}`);
        lines.push("");
    }
    if (output.plan.feedbackAddressed.length > 0) {
        lines.push("## Feedback Applied", "");
        for (const f of output.plan.feedbackAddressed)
            lines.push(`- ${f}`);
        lines.push("");
    }
...
async function readBriefChecklistState(briefPath) {
    let raw = "";
    try {
        raw = await readFile(briefPath, "utf8");
    }
    ...
    for (const line of raw.split("\n")) {
        const trimmed = line.trim();
        if (trimmed === "## Investor panel refinement (priority)")
            section = "investor";
        else if (trimmed === "### Must Ship (Phase 1)")
            section = "must";
        else if (trimmed === "### Should Ship (stretch)")
            section = "should";
        else if (trimmed === "## Unresolved Gaps (Cursor should fix these)")
        ...
        if (briefIntentToken(cleanText) === "" && cleanText === "")
            continue;
        checkedIds.add(mintBriefItemId(section, cleanText));
    }
    return { checkedIds };
}
function generateCursorBriefFromBuildSpec(projectName, buildSpec, audit, feedback, investorRefinement, previousChecklist, ledger) {
    const checkbox = (section, text, defaultChecked = false) => {
    ...
    const ledgerClosed = (taskId) => Boolean(ledger?.tasks && taskId in ledger.tasks);
    ...
    if (buildSpec.diagnostics.directivesWithoutTasks.length > 0) {
        lines.push("> **Wizard diagnostic:** the following directives could NOT be decomposed to concrete tasks this cycle and are deferred:", "", ...buildSpec.diagnostics.directivesWithoutTasks.map((d) => `> - ${d}`), "");
    }
    if (investorRefinement) {
        lines.push("## Investor panel refinement (priority)", "", `Round ${investorRefinement.round} — address these in the primary slice:`, "", ...investorRefinement.directives.map((d) => checkbox("investor", d)), "");
    }
    lines.push(`## Primary slice — ${primary.title}`, "", `**User story:** ${primary.userStory}`, "");
    ...
    if (buildSpec.parentDirectives.length > 0) {
        lines.push("### Parent directives (each closes when all its child tasks are checked)", "", ...buildSpec.parentDirectives.map((p) => `- **${p.id}** (${p.source}) — ${p.text}\n  - children: ${p.childTaskIds.join(", ") || "_none yet_"}`), "");
    }
    ...
    lines.push("---", "", "## How to use", "", "1. Read `.foundry/BUILD_SPEC.md` for full context", "2. Implement the primary slice acceptance criteria in product code", "3. Mark items `- [x]` only when implemented and tested", "");
    return lines.join("\n");
}
function generateCursorBrief(projectName, audit, productDef, monetization, plan, allActions, feedback, autos, investorRefinement, previousChecklist) {
    const checkbox = (section, text, defaultChecked = false) => {
        const cleanText = stripBriefIdComment(text);
        const id = mintBriefItemId(section, cleanText);
    ...
    if (investorRefinement) {
        lines.push("## Investor panel refinement (priority)", "", `Round ${investorRefinement.round} — do these before other checklist items:`, "", ...investorRefinement.directives.map((d) => checkbox("investor", d)), "", "```text", investorRefinement.investorSummaries.trim() || "(no summaries)", "```", "");
    }
    ...
    if (created.length > 0) {
        ...
        for (const a of created)
            lines.push(`- [x] \`${a.file}\` — ${a.description}`);
        lines.push("");
    }
    ...
    if (monetization?.gates && monetization.gates.length > 0) {
        const allPaywallVerified = !!autos && monetization.gates.every((g) => autos.paywallGatesDone.has(g.feature));
        lines.push("## Monetization Integration", "");
        if (allPaywallVerified) {
            lines.push("_All monetization gates are verified wired in repo (pattern scan on target screens). No open paywall checklist — change gates in `monetization_architect` / product config if you add new ones._", "");
        }
        else {
    ...
    if (edgeFnsForRateLimit.length > 0) {
        ...
        if (allEdgeRlVerified) {
            lines.push("_Rate limiting verified on all deployed edge handlers. No open wiring checklist._", "");
        }
        else {
...
function summarizeResolverActions(actions) {
    return {
...
export const builderStage = {
    ...
    async run(ctx, input) {
        ...
        if (!gitStatus.clean) {
            notes.push("Foundry detected existing non-generated working-tree changes and will adopt them into the Foundry branch commit instead of blocking.");
        }
        ...
        const isNode = (repoParsed.success && (repoParsed.data.summary?.repoTypeGuess === "node" || repoParsed.data.summary?.repoTypeGuess === "expo_rn")) ||
        ...
        const adaptiveMemory = await upsertAdaptiveDirectives(repoPath, ctx.runId, [
            ...gapActions
                .filter((g) => g.resolver === "none")
                .map((g) => ({ summary: g.gap, source: "gap_unresolved", likelyFiles: g.likelyFiles })),
            ...feedbackActions
                .filter((f) => !f.relatedResolver)
                .map((f) => ({ summary: f.summary, source: "feedback_unresolved", likelyFiles: f.likelyFiles })),
            ...openBriefItems
                .filter((item) => item.section !== "monetization" && item.section !== "edge")
                .map((item) => ({
                summary: item.text,
                source: "brief_open_item",
        ...
        notes.push(`Adaptive resolver memory updated: ${activeAdaptiveDirectives.length} active (${adaptiveDirectives.length} loaded this run) tracked in ${ADAPTIVE_RESOLVER_MEMORY_REL}.`);
        const screens = (audit?.screens ?? []);
        ...
        const paywallGatesDone = monetization?.gates && screens.length > 0
            ? await computeSatisfiedPaywallGates(repoPath, screens, monetization.gates)
            : new Set();
        ...
        const monetizationNeedsResolverPass = Boolean(monetization?.gates?.length &&
            (!paywallLibReady ||
                !analyticsLibReady ||
                (screens.length > 0 && paywallGatesDone.size < monetization.gates.length)));
        const edgeNeedsResolverPass = Boolean(edgeFunctionsDeployable.length > 0 && edgeRatePathsDone.size < edgeFunctionsDeployable.length);
        const coreProductBriefOpen = openBriefItems.filter((i) => i.section === "must" || i.section === "should" || i.section === "gaps" || i.section === "runtime").length;
        ...
        let plan = buildPlan(gapActions, feedbackActions, adaptiveDirectives, !!(monetization?.gates?.length), monetizationNeedsResolverPass, edgeFunctions.length > 0, edgeNeedsResolverPass, isNode, {
            coreFocusMode,
        ...
        if (!ENABLE_DETERMINISTIC_RESOLVERS) {
            if (plan.resolversToRun.size > 0) {
                notes.push(`Deterministic resolvers disabled for Cursor-first loop: deferred ${plan.resolversToRun.size} resolver(s) to Cursor implementation.`);
            }
        ...
        if (!allowEdgeInfrastructureResolvers) {
            notes.push("Builder deferred edge rate-limit codegen: no open items exist under ## Edge Function Rate Limiting in CURSOR_BRIEF.");
        }
        if (!allowMonetizationInfrastructureResolvers) {
            notes.push("Builder deferred paywall/analytics codegen: no open items exist under ## Monetization Integration in CURSOR_BRIEF.");
        }
        // ---- 4. Create or reuse branch for changes ----
        const stableBranch = input.config.project.foundry?.builder_branch?.trim();
        const branchName = stableBranch && stableBranch.length > 0 ? stableBranch : `foundry/${ctx.runId}`;
        ...
        if (branchCreated) {
            notes.push(stableReleaseBranch
        ...
        // ---- 5. Run resolvers ----
        const recordResolver = (name, actions, meta) => {
        ...
        if (plan.resolversToRun.has("paywall") && monetization?.gates) {
            const gates = monetization.gates;
        ...
        if (plan.resolversToRun.has("analytics") && monetization?.analyticsEvents) {
            const events = monetization.analyticsEvents;
        ...
        const skipBriefRegen = openBriefItems.length === 0 && (await fileExists(briefPath));
        const cursorBrief = skipBriefRegen
            ? await readFile(briefPath, "utf8")
            : gwParsed.success
                ? generateCursorBriefFromBuildSpec(projectName, gwParsed.data, audit, feedback, input.investorRefinement, previousChecklist, buildSpecLedger)
                : generateCursorBrief(projectName, audit, pdParsed.success ? pdParsed.data : undefined, monetization, plan, allFileActions, feedback, { paywallGatesDone, edgeRatePathsDone }, input.investorRefinement, previousChecklist);
        if (!skipBriefRegen) {
            await writeStageMarkdown(ctx, "builder", "CURSOR_BRIEF.md", cursorBrief);
            await mkdir(join(repoPath, ".foundry"), { recursive: true });
            await writeFile(briefPath, cursorBrief, "utf8");
            notes.push(gwParsed.success
            ...
            notes.push("CURSOR_BRIEF has no open tracked checklist items — skipped brief regen to avoid contract/test oscillation.");
        }
        if (!skipBriefRegen) {
            healCursorBriefContractFromRepoScript(repoPath, ctx.logger);
        }
        else {
            notes.push("Skipped brief contract heal — brief unchanged (no open checklist items).");
        }
        // ---- 8. Determine status ----
        ...
        if (unresolved.length > 0 && created.length + modified.length === 0) {
            finalStatus = "partial";
            notes.push("No deterministic resolver matched the remaining gaps. Handoff to Cursor via .foundry/CURSOR_BRIEF.md or add builder.directives in project.yaml for custom implementation.");
        }
        ...
        if (branchCreated) {
            notes.push(`Changes are on branch '${branchName}'. ` +
        ...
        await writeStageMarkdown(ctx, "builder", "README.md", generateSummary(validated, projectName, gitStatus.branch, gitStatus.headSha));
        return validated;
    },
};
/** Repos like GutCheck pin brief contract lines in product code — heal after wizard regen. */
function healCursorBriefContractFromRepoScript(repoPath, logger) {
