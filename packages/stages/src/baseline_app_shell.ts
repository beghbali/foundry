...
export type BaselineAppShellOutput = z.infer<typeof BaselineAppShellOutputSchema>;
...
function feedbackPlan(d: Detected, projectName: string): z.infer<typeof PlanBlockSchema> {
  ...
  if (d.tabsLayout) files.push(d.tabsLayout);
  if (d.rootLayout) files.push(d.rootLayout);
...
function authPlan(d: Detected, projectName: string): z.infer<typeof PlanBlockSchema> {
  const root = d.mobileRoot === "." ? "" : (d.mobileRoot ?? "") + "/";
  const ext = d.language === "js" ? "js" : "ts";
  const extx = d.language === "js" ? "jsx" : "tsx";
...
function profilingPlan(d: Detected, projectName: string): z.infer<typeof PlanBlockSchema> {
  const root = d.mobileRoot === "." ? "" : (d.mobileRoot ?? "") + "/";
  const ext = d.language === "js" ? "js" : "ts";
  const extx = d.language === "js" ? "jsx" : "tsx";
  ...
  if (d.settingsScreen) files.push(d.settingsScreen);
...
function buildReadme(
  ...
  lines.push(
    "**Templates** (copy from foundry and adapt):",
    `- \`${templateRoot}/feedback/FeedbackModal.tsx\``,
    `- \`${templateRoot}/feedback/useShakeToReport.ts\``,
    `- \`${templateRoot}/feedback/feedbackApi.ts\``,
    `- \`${templateRoot}/feedback/types.ts\``,
    "",
    "**Acceptance criteria:**",
    ...output.plan.feedback.acceptance.map((a) => `- [ ] ${a}`),
    "",
    "---",
    "",
    "## B) Authentication",
    "",
    `**Approach:** ${output.plan.auth.approach}`,
    "",
    "**Files to create/modify:**",
    ...output.plan.auth.files.map((f) => `- \`${f}\``),
    "",
  );
  ...
  lines.push(
    "**Templates:**",
    `- \`${templateRoot}/auth/AuthGate.tsx\``,
    `- \`${templateRoot}/auth/SignInScreen.tsx\``,
    `- \`${templateRoot}/auth/authClient.ts\``,
    "",
    "**Acceptance criteria:**",
    ...output.plan.auth.acceptance.map((a) => `- [ ] ${a}`),
    "",
    "---",
    "",
    "## C) Progressive Profiling",
    "",
    `**Approach:** ${output.plan.progressiveProfiling.approach}`,
    "",
    "**Files to create/modify:**",
    ...output.plan.progressiveProfiling.files.map((f) => `- \`${f}\``),
    "",
    "**Acceptance criteria:**",
    ...output.plan.progressiveProfiling.acceptance.map((a) => `- [ ] ${a}`),
    "",
  );
...
export const baselineAppShellStage: Stage<StageInputComposition, BaselineAppShellOutput> = {
  name: "baseline_app_shell",
  description:
    "Generate a project-specific implementation plan for feedback capture, authentication, and progressive profiling in an Expo RN app.",
  inputSchema: StageInputCompositionSchema,
  outputSchema: BaselineAppShellOutputSchema,
  async run(ctx: RunContext, input: StageInputComposition): Promise<BaselineAppShellOutput> {
    ...
    const repo = RepoHintSchema.safeParse(input.repoInventory);
    const isExpo =
      (repo.success && repo.data.summary?.hasExpo === true) ||
      (repo.success && repo.data.summary?.repoTypeGuess === "expo_rn") ||
      !!d.mobileRoot;

    if (!isExpo) {
      const output: BaselineAppShellOutput = {
        applicable: false,
        detected: {
          mobileRoot: d.mobileRoot,
          authPresent: d.authPresent,
    ...
    const applyFlag = (input.config.project as Record<string, unknown>).foundry;
    const shouldApply =
      typeof applyFlag === "object" &&
      applyFlag !== null &&
      (applyFlag as Record<string, unknown>).apply_baseline_shell === true;
