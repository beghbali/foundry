...
function addImport(content, importLine) {
    if (content.includes(importLine.replace(/;?\s*$/, "")))
        return content;
...
function networkStatusHookTemplate() {
...
function offlineBannerTemplate() {
    return `import React from 'react';
...
function paywallTemplate(gates, projectName) {
...
function analyticsTemplate(events, projectName) {
...
// ---------- auth guard resolver ----------
export async function resolveAuthGuards(repoPath, dynamicRoutes) {
    const actions = [];
    const GUARD_MARKER = "// [foundry:auth-guard]";
    const authGuardAlreadyPresent = (content) => {
        if (content.includes(GUARD_MARKER))
            return true;
        const hasAuthStateGuard = /const\s*\{\s*[^}]*isAuthenticated[^}]*isAnonymous[^}]*\}\s*=\s*useAuthStore\(\)/.test(content) &&
            /replace\(\s*['"]\/auth['"]\s*\)/.test(content) &&
            /!\s*isAuthenticated\s*&&\s*!\s*isAnonymous/.test(content);
        if (hasAuthStateGuard)
            return true;
        const hasLegacySessionGuard = /const\s*\{\s*session\s*\}\s*=\s*useAuthStore\(\)/.test(content) &&
            /replace\(\s*['"]\/auth['"]\s*\)/.test(content);
        ...
        if (fnMatch) {
            ...
            if (!modified.includes("import { useEffect") && !modified.includes("import {useEffect")) {
                if (modified.includes("from 'react'")) {
                    modified = modified.replace(/import \{([^}]+)\} from 'react'/, (match, imports) => {
...
export async function resolveInputValidation(repoPath, screenFiles) {
    const actions = [];
 ...
 */
const GATE_FEATURE_TO_SCREEN_FILES = {
...
const GATE_FEATURE_TO_SCREEN_KEYWORDS = {
...
function paywallFeatureRegex(feature) {
    return new RegExp(`shouldShowPaywall\\s*\\(\\s*['"]${feature.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}['"]`);
}
/** True if this source already enforces the gate (inline or prior Foundry wiring), so stubs must not be re-injected. */
export function paywallGateAlreadyWiredInSource(content, feature) {
    if (!content)
        return false;
    if (content.includes(PAYWALL_WIRE_MARKER) && content.includes(feature))
        return true;
    if (paywallFeatureRegex(feature).test(content))
        return true;
    if (new RegExp(`\\b_check_${feature.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\b`).test(content))
        return true;
    return false;
}
/** True if analytics for this event is already present (inline `track(...)` or Foundry marker). */
export function analyticsEventAlreadyWiredInSource(content, eventName) {
    if (!content)
        return false;
    if (content.includes(ANALYTICS_WIRE_MARKER) && content.includes(eventName))
        return true;
    const esc = eventName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    if (new RegExp(`\\btrack\\s*\\(\\s*['"]${esc}['"]`).test(content))
        return true;
    if (new RegExp(`\\b_track_${esc}\\b`).test(content))
        return true;
    return false;
}
function targetScreensForPaywallGate(screens, feature) {
    ...
    return screens.filter((s) => matchNames.some((m) => s.name.toLowerCase().includes(m.toLowerCase()) || s.file.toLowerCase().includes(m.toLowerCase())));
}
/**
 * Gate features whose target screens all already contain real or stub wiring — use for CURSOR_BRIEF `[x]`.
 */
export async function computeSatisfiedPaywallGates(repoPath, screens, gates) {
    ...
    for (const gate of gates) {
        ...
        if (allOk)
            satisfied.add(gate.feature);
    }
    return satisfied;
}
const ANALYTICS_EVENT_SCREEN_KEYWORDS = {
    paywall_shown: ["pro"],
    paywall_dismissed: ["pro"],
    trial_started: ["pro"],
};
function targetScreensForAnalyticsEvent(screens, eventName) {
    const keywords = ANALYTICS_EVENT_SCREEN_KEYWORDS[eventName];
    if (!keywords)
        return [];
    return screens.filter((s) => keywords.some((kw) => s.name.toLowerCase().includes(kw) || s.file.toLowerCase().includes(kw)));
}
/** Event names that are already wired on all target screens. */
export async function computeSatisfiedAnalyticsEvents(repoPath, screens, events) {
    const satisfied = new Set();
    for (const event of events) {
        ...
        if (allOk)
            satisfied.add(event.name);
    }
    return satisfied;
}
/** Edge function `path/index.ts` paths that already include Foundry rate-limit wiring. */
export async function computeSatisfiedEdgeRateLimitPaths(repoPath, edgeFunctions) {
    const done = new Set();
    ...
    for (const fn of edgeFunctions) {
        ...
        if (hasMarker || (hasImport && hasUsage))
            done.add(rel);
    }
    return done;
}
export async function wirePaywallIntoScreens(repoPath, screens, gates) {
    const actions = [];
    for (const gate of gates) {
        ...
        for (const screen of targetScreens) {
            ...
            actions.push({
                file: screen.file,
                action: "skipped",
                description: `Paywall gate ${gate.feature} still needs inline wiring in ${screen.file}; ` +
...
export async function wireAnalyticsIntoScreens(repoPath, screens, events) {
    const actions = [];
    for (const event of events) {
        ...
        for (const screen of targetScreens) {
            ...
            actions.push({
                file: screen.file,
                action: "skipped",
                description: `Analytics event ${event.name} still needs inline wiring in ${screen.file}; ` +
...
export async function wireRateLimitIntoEdgeFunctions(repoPath, edgeFunctions) {
    const actions = [];
    const RL_MARKER = "// [foundry:rate-limit-wired]";
    for (const fn of edgeFunctions) {
        if (fn.name === "_shared")
...
export async function resolveErrorBoundary(repoPath, mobileRoot) {
    const actions = [];
...
export async function resolveOfflineHandler(repoPath, mobileRoot) {
    const actions = [];
    const base = mobileRoot === "." ? repoPath : join(repoPath, mobileRoot);
...
export async function resolveRateLimiting(repoPath, edgeFunctions, backendRoot) {
    const actions = [];
    if (!backendRoot || edgeFunctions.length === 0)
        return actions;
...
export async function resolvePaywallGates(repoPath, mobileRoot, gates, projectName) {
    const actions = [];
    if (gates.length === 0)
        return actions;
    const base = mobileRoot === "." ? repoPath : join(repoPath, mobileRoot);
...
export async function resolveAnalytics(repoPath, mobileRoot, events, projectName) {
    const actions = [];
    if (events.length === 0)
        return actions;
    const base = mobileRoot === "." ? repoPath : join(repoPath, mobileRoot);
