...
export async function ensureBranch(cwd, name) {
...
export async function diff(cwd) {
    ...
    for (const line of result.stdout.split("\n")) {
        ...
        if (!file)
            continue;
        if (isGeneratedFoundryPath(file))
            continue;
...
export async function relevantPaths(cwd) {
...
export async function addRelevant(cwd) {
...
export async function hasRemote(cwd) {
    ...
    if (ghMatch)
        return { hasGithub: true, remote: ghMatch[1] };
    return { hasGithub: false };
}
export async function switchBack(cwd, branch) {
