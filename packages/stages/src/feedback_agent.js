...
async function readTextSafe(abs, maxBytes = MAX_TEXT_BYTES) {
    try {
...
async function readFeedbackLedger(repoPath) {
    ...
    try {
        const parsed = FeedbackLedgerSchema.safeParse(JSON.parse(raw));
        if (parsed.success)
            return parsed.data;
    }
    catch {
...
async function readPriorFeedbackOutput(repoPath, currentRunId) {
...
async function collectSupabase(ctx) {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const summary = (typeof row.message === "string" && row.message) ||
        ...
        let type = "complaint";
        if (rawType.includes("crash") || rawSeverity.includes("crash") || rawSeverity.includes("fatal"))
            type = "crash";
        else if (rawType.includes("bug") || rawSeverity.includes("bug") || rawSeverity.includes("error"))
            type = "bug";
        else if (rawType.includes("feature") || rawType.includes("request") || rawSeverity.includes("idea"))
            type = "feature_request";
        else if (rawType.includes("praise") || rawType.includes("positive"))
            type = "praise";
        else if (rawType.includes("complaint"))
            type = "complaint";
        let priority = "medium";
        if (type === "crash" ||
        ...
        const ts = typeof row.created_at === "string"
            ? row.created_at
    ...
    if (/\b(eas[- ]build|xcode_build_error|expo build|build failed|build command failed|app config|config plugin)\b/i.test(lower)) {
        if (REPO_FIXABLE_BUILD_ERROR_RE.test(lower)) {
            return {
                ...
                implementationNote: "EAS/Xcode/Expo build log contains repo-fixable compile/bundle/runtime error — queued for Cursor.",
            };
        }
        return {
            repoActionable: true,
            shouldImplement: false,
            implementationNote: "EAS/Xcode/Expo build failed but the log did not match a known repo-fixable pattern; review the log manually and mark shouldImplement=true if it is actionable.",
        };
    }
...
function mergeLedger(existing, collected, collectedAt) {
    const priorById = new Map(existing.items.map((item) => [item.id, item]));
    const merged = [];
    const isGeneratedSource = (source) => GENERATED_FEEDBACK_SOURCE_PREFIXES.some((prefix) => source.startsWith(prefix));
    for (const raw of collected) {
        ...
        const status = prior?.status === "ignored"
            ? "ignored"
            : prior?.status === "resolved" && sameGeneratedSignal
                ...
                : prior && generatedPrior
                    ? "open"
                    : (prior?.status ?? "open");
        const shouldRefreshNote = !prior?.implementationNote ||
    ...
    for (const prior of existing.items) {
        if (merged.some((item) => item.id === prior.id))
            continue;
...
async function collectManualFeedback(repoPath) {
        ...
        if (name.toLowerCase().endsWith(".json")) {
            try {
                ...
                const pushOne = (obj, idx) => {
                    ...
                    let type;
                    if (tRaw === "feature_request" || tRaw === "feature")
                        type = "feature_request";
                    else if (tRaw === "bug")
                        type = "bug";
                    else if (tRaw === "praise")
                        type = "praise";
                    else if (tRaw === "complaint")
                        type = "complaint";
                    else if (tRaw === "crash")
                        type = "crash";
                    else {
                    ...
                    if (pr === "high" || pr === "low" || pr === "medium")
                        ...
                        priority = classifyFromText(summary).priority;
                    items.push({
...
async function collectCrashLogs(repoPath, ctx) {
    const items = [];
    const roots = [
        repoPath,
        join(repoPath, ".expo"),
    ...
    for (const root of roots) {
        for (const fname of CRASH_LOG_CANDIDATES) {
            ...
            if (!text || text.length < 40)
                continue;
            if (!/\b(Error|Exception|FATAL|crash|SIG|fatal)\b/i.test(text))
                continue;
    ...
    try {
        ...
        for (const name of expoFiles) {
            ...
            if (!text || text.length < 40)
                continue;
            if (!/\b(Error|Exception|FATAL|crash)\b/i.test(text))
                continue;
            const snippet = text.split(/\r?\n/).slice(0, 20).join("\n").slice(0, 1500);
            items.push({
 ...
 */
function extractExpoBuildViewSummary(text) {
    ...
    if (stdoutMatch?.[1])
        candidates.push(stdoutMatch[1].trim());
    ...
    if (Array.isArray(parsed.logFiles))
        push("logFileCount", parsed.logFiles.length);
...
async function collectFoundryLogs(repoPath, ctx) {
    const files = [];
    const roots = [
    ...
    for (const abs of files) {
        ...
        if (!isExpoBuildView &&
            !/\b(crash|fatal|exception|error|build failed|build command failed|failed to read the app config|valid config plugin|unexpected token|xcode_build_error|app init error)\b/i.test(text))
            continue;
        let snippet = text;
        if (isExpoBuildView) {
        ...
        const isHighPriority = isExpoBuildView ||
    ...
    if (items.length)
        ctx.logger(`[feedback_agent] collected ${items.length} foundry log signal(s)`);
    return items;
}
function collectPipelineSignals(input) {
    void input;
    return [];
}
function crossReference(items, gaps, acs) {
    return items.map((it) => {
...
function buildPatterns(items) {
    ...
    for (const [theme, group] of buckets) {
        ...
        patterns.push({
            ...
            count: group.length,
            severity,
            suggestedAction: hasCrash || bugN > 0
...
function buildNextHints(items, gaps) {
    const prioritizeGaps = [];
    for (const g of gaps) {
        const hit = items.some((it) => {
            if (!it.relatedGap || it.relatedGap !== g.description)
                return false;
            return it.type === "bug" || it.type === "crash" || it.type === "complaint";
        });
        if (hit)
            prioritizeGaps.push(`[${g.area}] ${g.description}`);
    }
    const noisyAreas = new Set();
    for (const g of gaps) {
        const hit = items.some((it) => it.relatedGap === g.description &&
            (it.type === "bug" || it.type === "crash" || it.type === "complaint"));
        if (hit)
            noisyAreas.add(g.area);
    }
...
function computeTrends(items, prior) {
    ...
    let trend = "stable";
    if (bugCount < previousRunBugCount)
        trend = "improving";
    else if (bugCount > previousRunBugCount)
        trend = "worsening";
    return {
        totalFeedback,
        bugCount,
...
function buildReadme(output, projectName) {
    const lines = [
        ...
        `- Trend vs last run: **${output.trends.trend}**${output.trends.previousRunBugCount !== null
    ...
    if (output.nextPipelineHints.prioritizeGaps.length === 0)
        lines.push("_None._");
    else
        for (const g of output.nextPipelineHints.prioritizeGaps)
            lines.push(`- ${g}`);
    lines.push("", "### Skip or defer areas (low negative signal)");
    if (output.nextPipelineHints.skipAreas.length === 0)
        lines.push("_None._");
    else
        for (const a of output.nextPipelineHints.skipAreas)
            lines.push(`- ${a}`);
    lines.push("", "### New feature requests");
    if (output.nextPipelineHints.newFeatureRequests.length === 0)
        lines.push("_None._");
    else
        for (const f of output.nextPipelineHints.newFeatureRequests)
            lines.push(`- ${f}`);
    lines.push("", "## Sample items");
    for (const it of output.items.slice(0, 15)) {
        lines.push(`- [${it.type}/${it.priority}] [${it.status}] implement=${it.shouldImplement ? "yes" : "no"} ${it.summary.slice(0, 200)}${it.summary.length > 200 ? "…" : ""}`);
    }
    if (output.items.length > 15)
        lines.push(`_…and ${output.items.length - 15} more._`);
    return lines.join("\n");
}
export const feedbackAgentStage = {
    name: "feedback_agent",
    description: "Aggregate external feedback into a durable ledger; emit only open ledger items and implementation hints for the next run.",
    ...
    async run(ctx, input) {
        ...
        let items = [...supa.items, ...manual, ...crashItems, ...foundryLogs];
        const seen = new Set();
