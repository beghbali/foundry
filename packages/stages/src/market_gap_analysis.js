...
export const MarketGapOutputSchema = z.object({
    ...
    gapsToExploit: z.array(z.object({
        gap: z.string(),
        whyNow: z.string(),
        howWeWin: z.string(),
    })),
...
function normalizeCompetitors(fromConfig, projectName, domain) {
...
function pickDifferentiatorHints(differentiators) {
...
function buildOutput(input) {
    ...
    const commonComplaints = domain === "gardening"
        ? [
            {
                theme: "Advice is too generic for climate, season, and garden setup",
                whyItMatters: "Garden outcomes depend heavily on location, seasonality, sun exposure, and whether the user is growing in beds, containers, or indoors.",
                userImpact: "Users lose trust when recommendations do not match what they can actually plant or do this week.",
            },
            {
                theme: "Logging and garden planning takes too much effort",
                whyItMatters: "Gardeners need lightweight capture during busy moments like planting, watering, pruning, and harvest.",
                userImpact: "Users stop tracking after the first few sessions, which breaks habit loops and reduces retention.",
            },
            {
                theme: "Diagnosis stops at identification instead of next-best action",
                whyItMatters: "Gardeners need confidence about what to do next, not just what a pest, deficiency, or symptom might be.",
                userImpact: "Users still leave the app to search forums, YouTube, or Reddit before taking action.",
            },
            {
                theme: "No strong week-by-week guidance for what matters now",
                whyItMatters: "Gardening is seasonal and task-driven, so users need timely recommendations rather than static reference content.",
                userImpact: "Engagement fades between planting moments because the app does not create a recurring rhythm.",
            },
            {
                theme: "Garden tools feel fragmented across planning, care, and harvest",
                whyItMatters: "Users want one place to decide what to plant, track what exists, and respond to problems as they arise.",
                userImpact: "They juggle notes, reminders, photos, and external communities instead of building habit inside the app.",
            },
        ]
        : [
            {
    ...
    const gapsToExploit = domain === "gardening"
        ? [
            {
                gap: "What should I do in my garden this week?",
                whyNow: "Most gardening apps skew toward reference content or care reminders, but gardeners return for timely, seasonal decisions.",
                howWeWin: "Anchor the product around a weekly action feed personalized by garden type, season, and current plant stage.",
            },
            {
                gap: "Decision support tailored to the gardener's style and experience level",
                whyNow: "Beginners need confidence and simplification, while experienced gardeners want nuance without losing speed.",
                howWeWin: `Use gardener profile + context to adapt recommendations, tone, and defaults around ${diffs[0]}.`,
            },
            {
                gap: "From problem detection to concrete recovery plan",
                whyNow: "Plant diagnosis is increasingly commoditized; execution guidance is where trust and retention are earned.",
                howWeWin: "Turn every issue into a short recovery workflow with likely causes, immediate steps, follow-up checks, and expected timeline.",
            },
            {
                gap: "Simple garden system of record that does not feel like data entry",
                whyNow: "Gardeners will log if it directly improves recommendations, but abandon apps that feel like admin work.",
                howWeWin: "Capture state through quick actions, photos, and inferred events so the app gets smarter with minimal effort.",
            },
            {
                gap: hasSupabase
                ...
                whyNow: "Winning products compound from localized, repeated user outcomes rather than static care libraries.",
                howWeWin: "Track which recommendations led to success, failure, or follow-up questions, then tune weekly guidance around real outcomes.",
            },
        ]
        : [
            {
    ...
    const mustNotDo = domain === "gardening"
        ? [
...
export const marketGapAnalysisStage = {
    name: "market_gap_analysis",
    description: "Mode A (default): deterministic market gap synthesis from project config, differentiators, and local repo signals.",
    ...
    async run(ctx, input) {
        ...
        await writeStageMarkdown(ctx, "market_gap_analysis", "README.md", buildReadme(validated, input.config.project.project_name));
        return validated;
    },
};
