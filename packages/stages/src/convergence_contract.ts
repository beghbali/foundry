...
export type ProductLedgerItem = z.infer<typeof ProductLedgerItemSchema>;
...
export type ProductLedgerFile = z.infer<typeof ProductLedgerFileSchema>;
...
export type ConvergenceContractOutput = z.infer<typeof ConvergenceContractOutputSchema>;
...
export function manualResolutionsPath(repoPath: string): string {
...
export type ManualResolutionsFile = z.infer<typeof ManualResolutionsFileSchema>;

export async function readManualResolutions(repoPath: string): Promise<ManualResolutionsFile> {
  try {
    const raw = await readFile(manualResolutionsPath(repoPath), "utf8");
    const parsed = ManualResolutionsFileSchema.safeParse(JSON.parse(raw) as unknown);
...
export async function writeManualResolutions(repoPath: string, file: ManualResolutionsFile): Promise<void> {
  const dir = join(repoPath, ".foundry");
  await mkdir(dir, { recursive: true });
  await writeFile(manualResolutionsPath(repoPath), `${JSON.stringify(file, null, 2)}\n`, "utf8");
}

async function readProductLedger(repoPath: string): Promise<ProductLedgerFile> {
  try {
    const raw = await readFile(ledgerPath(repoPath), "utf8");
    const parsed = ProductLedgerFileSchema.safeParse(JSON.parse(raw) as unknown);
    if (parsed.success) return parsed.data;
  } catch {
    /* fall through */
  }
...
async function writeProductLedger(repoPath: string, file: ProductLedgerFile): Promise<void> {
  await mkdir(join(repoPath, ".foundry"), { recursive: true });
  await writeFile(ledgerPath(repoPath), JSON.stringify(file, null, 2), "utf8");
}

type LedgerSeed = Pick<ProductLedgerItem, "name" | "source" | "status" | "reason"> & {
...
function mergeLedger(
  prior: ProductLedgerFile,
  seeds: LedgerSeed[],
  refinementRound: number,
): ProductLedgerFile {
  const ts = nowIso();
  const byId = new Map<string, ProductLedgerItem>();
  for (const item of prior.items) byId.set(item.id, item);
  ...
  for (const seed of seeds) {
    ...
    if (existing) {
      byId.set(id, {
        ...
        linkedObjections: Array.from(
          new Set([...(existing.linkedObjections ?? []), ...(seed.linkedObjections ?? [])]),
        ),
        lastSeenAt: ts,
        refinementRound,
      });
  ...
  const buckets: Record<LedgerStatus, ProductLedgerItem[]> = {
    core_mvp: [],
    evidence_needed: [],
    later_expansion: [],
    do_not_build_yet: [],
  ...
  for (const item of byId.values()) buckets[item.status].push(item);
  for (const key of Object.keys(buckets) as LedgerStatus[]) {
    buckets[key].sort((a, b) => (b.lastSeenAt > a.lastSeenAt ? 1 : -1));
    buckets[key] = buckets[key].slice(0, MAX_LEDGER_RETENTION_PER_BUCKET);
  }

  const merged = ([] as ProductLedgerItem[]).concat(...Object.values(buckets));
...
function objectionSeed(text: string): string {
  return slugify(
    text
...
const POSITIVE_PHRASES =
  /^(yes|the best|interesting|promising|useful|great|strong|love|excited|exciting|solid|smart|nice|wonderful)/i;
 ...
 * effective.
 */
function collectObjectionSeeds(
  panel: z.infer<typeof InvestorPanelHintSchema> | undefined,
  ir: InvestorRefinementContext | undefined,
): ObjectionSeed[] {
...
type PriorObjection = z.infer<typeof OpenObjectionSchema>;
 ...
 */
function fingerprintPanel(panel: z.infer<typeof InvestorPanelHintSchema> | undefined): string {
  if (!panel) return "";
  const stable = {
...
function reconcileObjections(
  prior: PriorObjection[],
  seeds: ObjectionSeed[],
  refinementRound: number,
  panelIsFresh: boolean,
  currentPanelFingerprint: string,
  priorPanelFingerprint: string,
): PriorObjection[] {
  const byId = new Map<string, PriorObjection>();
  for (const o of prior) byId.set(o.id, { ...o });
  ...
  for (const [id, o] of byId.entries()) {
    ...
    const isStaleGeneration =
      fp !== "" && fp !== currentPanelFingerprint && fp !== priorPanelFingerprint;
...
function rankObjections(items: PriorObjection[]): PriorObjection[] {
  const order: Record<PriorObjection["status"], number> = {
    regressed: 0,
    open: 1,
    reduced: 2,
    resolved: 3,
...
function autoResolveObjections(
  objections: PriorObjection[],
  draft: ContractDraft,
  config: StageInputComposition["config"],
  monetization: unknown,
): { objections: PriorObjection[]; evidence: Array<{ objectionId: string; evidence: string }> } {
  const evidence: Array<{ objectionId: string; evidence: string }> = [];
  ...
  const monAny = monetization as Record<string, unknown> | undefined;
  const monGates = Array.isArray(monAny?.gates) ? (monAny!.gates as Array<Record<string, unknown>>) : [];
  const monPricing = (monAny?.pricing as Record<string, unknown> | undefined) ?? {};
  const monAnalytics = Array.isArray(monAny?.analyticsEvents)
    ? (monAny!.analyticsEvents as Array<Record<string, unknown>>)
    : [];
  const tight = isContractNarrative_Tight(draft);

  for (const o of objections) {
    if (o.status === "resolved" || o.status === "reduced") continue;
    ...
    if (narrativePatterns.some((re) => re.test(text))) {
      ...
      if (draft.mustNotShipYet.length < 2) reasons.push(`parked=${draft.mustNotShipYet.length}<2`);
    ...
    const nameMap: Array<{ re: RegExp; needles: string[]; label: string }> = [
...
function requiredEvidenceFor(text: string): string {
...
type ContractInputs = {
  ...
  refinementRound: number;
  priorObjections: PriorObjection[];
  priorLedger: ProductLedgerFile;
};
...
type ContractDraft = {
  ...
  singularLoop: z.infer<typeof SingularLoopSchema>;
  mustShip: string[];
  mustNotShipYet: string[];
  parkedSeeds: LedgerSeed[];
  evidenceGates: z.infer<typeof EvidenceGateSchema>[];
};

function safeParseInputs(input: StageInputComposition) {
  return {
 ...
 */
function deriveNorthStarTarget(
  metricKey: string,
  directives: string,
  metricsYaml: Array<{ key: string; target: number }> | undefined,
  fallback?: string,
): string {
  ...
  if (ret) return `D${ret[1]} retention ≥ ${ret[2]}%`;
  const conv = dl.match(/(?:conversion|paywall)[^\d]{0,20}(\d+)\s*%/);
  if (conv) return `Paywall conversion ≥ ${conv[1]}%`;
  ...
  if (yamlMatch && Number.isFinite(yamlMatch.target)) {
    ...
    if (t > 0 && t <= 1) return `${(t * 100).toFixed(t < 0.1 ? 1 : 0)}%`;
    return String(t);
  }

  const k = metricKey.toLowerCase();
  if (/retention|d\d|repeat/.test(k)) return "D7 retention ≥ 25% on the singular MVP loop";
  if (/latency|speed|response|seconds|ms/.test(k)) return "≤ 2s loop latency on target hardware";
  if (/conversion|paywall|trial/.test(k)) return "Paywall conversion ≥ 4% post-trial";
  if (/acceptance|recommend/.test(k)) return "≥ 50% recommendation acceptance after first 3 loops";
  if (/completion|success|outcome/.test(k)) return "≥ 60% of sessions complete the primary loop end-to-end";
  if (/referral/.test(k)) return "≥ 1 referral per 10 retained users (Phase-1 cohort)";
  return fallback ?? "≥ 60% of Phase-1 cohort hits the metric weekly";
}

function buildSingularLoop(
  flywheel: z.infer<typeof FlywheelOutputSchema> | undefined,
  productDefinition: z.infer<typeof ProductDefinitionHintSchema> | undefined,
  northStar: string,
  ir: InvestorRefinementContext | undefined,
  metricsYaml: Array<{ key: string; target: number }> | undefined,
): z.infer<typeof SingularLoopSchema> {
...
function pickMustShip(
  productDefinition: z.infer<typeof ProductDefinitionHintSchema> | undefined,
  flywheel: z.infer<typeof FlywheelOutputSchema> | undefined,
  loopName: string,
): { mustShip: string[]; spilled: string[] } {
  const candidates: string[] = [];
  if (productDefinition) candidates.push(...productDefinition.scope.mustShip);
  if (flywheel?.focusRecommendation.phase1) candidates.push(...flywheel.focusRecommendation.phase1);
...
function looksParkable(text: string): boolean {
  return PARK_KEYWORDS.some((re) => re.test(text));
}

function buildDraft(args: ContractInputs): ContractDraft {
  ...
  const parkedSeeds: LedgerSeed[] = [];

  const addParked = (
    name: string,
    source: ProductLedgerItem["source"],
    reason: string,
    reentry?: string,
  ): void => {
    const slug = slugify(name);
    if (parkedNamesSet.has(slug)) return;
  ...
  for (const k of firstPrinciples?.kill_list ?? []) {
    addParked(
      k,
      "kill_list",
      "Listed in first_principles kill_list — must not enter Phase-1 scope.",
      "Revisit only after the singular MVP loop reaches its evidence gate.",
    );
  }
  ...
  for (const item of spilled) {
    addParked(
      item,
      "convergence_contract",
      "Exceeded MVP must-ship cap (max " + MAX_MUST_SHIP + ") — parked to keep one loop.",
      "Revisit after retention proof on the singular loop.",
    );
  }
  ...
  for (const item of productDefinition?.scope.shouldShip ?? []) {
    if (looksParkable(item)) {
      addParked(
        item,
        "convergence_contract",
        "Should-ship item matches a known distraction pattern — parked.",
        "Revisit after retention + monetization proof on the singular loop.",
      );
    }
  }
  ...
  for (const item of productDefinition?.scope.wontShip ?? []) {
    addParked(
      item,
      "convergence_contract",
      "Already excluded by product_definition.wontShip — preserved in ledger.",
      "Re-evaluate only with explicit evidence from the singular loop.",
    );
  }

  for (const item of flywheel?.focusRecommendation.phase2 ?? []) {
    if (parkedNamesSet.has(slugify(item))) continue;
    parkedSeeds.push({
      name: item,
  ...
  for (const d of ir?.directives ?? []) {
    if (looksParkable(d) || /delay|park|defer|strip|remove|cut/i.test(d)) {
      addParked(
        d,
        "investor_feedback",
        `Investor refinement r${ir?.round ?? 0} requested narrowing.`,
        "Revisit only when the singular loop is proven and an investor objection unblocks it.",
      );
    }
  }
  ...
  const evidenceGates: z.infer<typeof EvidenceGateSchema>[] = [
    {
...
function computeWarnings(
  draft: ContractDraft,
  pdInput: z.infer<typeof ProductDefinitionHintSchema> | undefined,
  panel: z.infer<typeof InvestorPanelHintSchema> | undefined,
  objections: PriorObjection[],
): string[] {
  ...
  if (pdInput && pdInput.coreWorkflows.length > draft.singularLoop.steps.length && pdInput.coreWorkflows.length > 1) {
    warnings.push(
      `product_definition declares ${pdInput.coreWorkflows.length} core workflows. MVP convergence requires 1.`,
    );
  }
...
function buildReadme(
  output: ConvergenceContractOutput,
  projectName: string,
): string {
  const lines: string[] = [
...
function readPriorObjections(input: StageInputComposition): PriorObjection[] {
...
function readPriorPanelFingerprint(input: StageInputComposition): string {
  const prior = ConvergenceContractOutputSchema.safeParse(input.convergenceContract);
  if (!prior.success) return "";
...
export const convergenceContractStage: Stage<StageInputComposition, ConvergenceContractOutput> = {
  name: "convergence_contract",
  description:
    "Foundry's MVP convergence engine: distill one product thesis, one loop, a parked product-ledger, evidence gates, and tracked investor objections. Downstream stages consume this contract.",
  inputSchema: StageInputCompositionSchema,
  outputSchema: ConvergenceContractOutputSchema,
  async run(ctx: RunContext, input: StageInputComposition): Promise<ConvergenceContractOutput> {
    ...
    let reconciledObjections = reconcileObjections(
      priorObjections,
      objectionSeeds,
      refinementRound,
      panelIsFresh,
    ...
    const autoResolve = autoResolveObjections(
    ...
    const manualEvidence: Array<{ objectionId: string; evidence: string }> = [];
    ...
    const ledgerSeeds: LedgerSeed[] = [
      ...draft.parkedSeeds,
      ...draft.mustShip.map<LedgerSeed>((name) => ({
        name,
    ...
    const counts = mergedLedger.items.reduce(
      (acc, item) => {
        acc[item.status]++;
        return acc;
      },
      { core_mvp: 0, evidence_needed: 0, later_expansion: 0, do_not_build_yet: 0, rejected: 0 } as Record<
        LedgerStatus,
        number
      >,
    );
    ...
    const output: ConvergenceContractOutput = {
