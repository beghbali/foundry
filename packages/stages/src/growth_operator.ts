...
import { BuilderOutputSchema, type BuilderOutput } from "./builder.js";
import { FlywheelOutputSchema } from "./flywheel_designer.js";
import { MonetizationOutputSchema } from "./monetization_architect.js";
import { ProductDefinitionOutputSchema, type ProductDefinitionOutput } from "./product_definition.js";
import { ReleaseAgentOutputSchema, type ReleaseAgentOutput } from "./release_agent.js";
...
export type GrowthOperatorOutput = z.infer<typeof GrowthOperatorOutputSchema>;

type ProductStage = GrowthOperatorOutput["productStage"];
type MetricStatus = GrowthOperatorOutput["metricsSnapshot"][number]["status"];

function isGardeningDomain(projectName: string, northStar: string): boolean {
...
async function countPriorApprovedReleases(repoPath: string, excludeRunId: string): Promise<number> {
...
function detectProductStage(release: ReleaseAgentOutput | undefined, priorApprovedReleases: number): ProductStage {
  const rel = release;
  if (
    rel?.status === "awaiting_approval" ||
  ...
  if (priorApprovedReleases === 0) return "pre_launch";
  if (priorApprovedReleases === 1) return "launched";
  return "growing";
}

function normalizeKeyParts(key: string): string[] {
  return key
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((p) => p.length > 2);
...
function haystackForImplementation(productDef: ProductDefinitionOutput, builder: BuilderOutput | undefined): string {
  const pdLines = [
...
function metricImplementationStatus(
  metricKey: string,
  haystack: string,
  flywheelMetricKeys: Set<string>,
): MetricStatus {
  ...
  for (const p of parts) {
    if (haystack.includes(p)) hits += 1;
  }
  if (flywheelMetricKeys.has(metricKey)) hits += 1;

  if (hits >= Math.min(2, parts.length)) return "on_track";
  if (hits === 0) return "at_risk";
  return "unknown";
}
...
function buildExperiments(
  ...
  northStar: string,
  flywheelMetrics: string[],
): GrowthOperatorOutput["experiments"] {
  ...
  if (productStage === "pre_launch") {
    pool = pool.map((e) =>
      e.name.includes("Paywall") || e.name.includes("Referral")
        ? { ...e, priority: Math.max(1, e.priority - 1) as 1 | 2 | 3 | 4 | 5 }
        : e,
    );
  }
...
function flywheelAlignmentRows(
  flyParsed: ReturnType<typeof FlywheelOutputSchema.safeParse>,
  productDef: ProductDefinitionOutput | undefined,
  builder: BuilderOutput | undefined,
): GrowthOperatorOutput["flywheelAlignment"] {
  ...
  return flyParsed.data.flywheel.map((loop) => {
    ...
    for (const must of productDef?.scope.mustShip ?? []) {
      ...
      if (loopKeywords.split(/\s+/).some((w) => w.length > 3 && m.includes(w))) score += 1;
    }
    if (hay.includes(metricKey.toLowerCase())) score += 2;
    if (builder) {
      const files = [...builder.changes.filesCreated, ...builder.changes.filesModified].join(" ").toLowerCase();
      if (files.includes(metricKey.split("_")[0] ?? "")) score += 1;
    }

    let healthSignal: "strong" | "weak" | "unmeasured" = "unmeasured";
    if (score >= 3) healthSignal = "strong";
    else if (score >= 1) healthSignal = "weak";
...
async function computeRunHistory(
  repoPath: string,
  currentRunId: string,
  currentSnapshot: GrowthOperatorOutput["metricsSnapshot"],
): Promise<GrowthOperatorOutput["runHistory"]> {
  ...
  if (priorIds.length > 0) {
    ...
    if (parsed.success) lastRunAt = parsed.data.startedAt;
  }
  ...
  if (priorScores.length >= 1) {
    ...
    if (priorScores.length >= 2) {
      ...
      if (current > Math.max(latestPrior, older)) trend = "improving";
      else if (current < Math.min(latestPrior, older)) trend = "declining";
      else trend = "stable";
    } else {
      if (current > latestPrior) trend = "improving";
      else if (current < latestPrior) trend = "declining";
      else trend = "stable";
    }
  }
...
function renderGrowthDashboard(output: GrowthOperatorOutput, projectName: string, northStar: string): string {
  const metricsTable = [
    "| Metric | Target | Current | Status |",
    "| --- | ---: | ---: | --- |",
    ...output.metricsSnapshot.map(
      (m) => `| \`${m.key}\` | ${m.target} | ${m.current === null ? "—" : m.current} | ${m.status} |`,
    ),
  ].join("\n");
  ...
  const experimentsTable = [
    ...
    "| ---: | --- | --- | --- | --- |",
    ...output.experiments.map(
      (e) => `| ${e.priority} | **${e.name}** | ${e.hypothesis} | \`${e.metric}\` | ${e.effort} |`,
    ),
  ].join("\n");
...
export const growthOperatorStage: Stage<StageInputComposition, GrowthOperatorOutput> = {
  ...
  async run(ctx: RunContext, input: StageInputComposition): Promise<GrowthOperatorOutput> {
    ...
    const releaseOut: ReleaseAgentOutput | undefined = releaseParsed.success ? releaseParsed.data : undefined;
    ...
    const flywheelMetricKeys = new Set<string>();
    if (flyParsed.success) {
    ...
    const builderOut: BuilderOutput | undefined = builderParsed.success ? builderParsed.data : undefined;
    const haystack = productParsed.success ? haystackForImplementation(productParsed.data, builderOut) : "";

    const metricsSnapshot: GrowthOperatorOutput["metricsSnapshot"] = input.config.metrics.metrics.map((m) => ({
      key: m.key,
      target: m.target,
      current: null,
    ...
    const productOut: ProductDefinitionOutput | undefined = productParsed.success ? productParsed.data : undefined;
    ...
    const headlineRec = domainPrimary
      ? `Align the next ${experiments.length} experiments with the configured primary moment: ${domainPrimary}${
          domainMetric ? ` (target: ${domainMetric})` : ""
        }.`
      : `Align the next ${experiments.length} experiments with north star: ${northStar.slice(0, 160)}${northStar.length > 160 ? "…" : ""}`;
    const recommendations: string[] = [headlineRec];
    ...
    if (gatedFeatures.length) {
      recommendations.push(
        `Monetization gates in play (${gatedFeatures.slice(0, 3).join(", ")}): pair growth tests with entitlement analytics.`,
      );
    }
    if (flyParsed.success) {
      recommendations.push(
        `Prioritize loops where health is weak: ${flywheelAlignment.filter((f) => f.healthSignal === "weak").map((f) => f.loop).join("; ") || "none flagged — keep monitoring"}.`,
      );
    }
    recommendations.push(
      `Run history trend is \`${runHistory.trend}\` across ${runHistory.totalRuns} pipeline run(s); add real metric ingestion to replace null currents.`,
    );

    const output: GrowthOperatorOutput = {
      metricsSnapshot,
      productStage,
      experiments,
      flywheelAlignment,
