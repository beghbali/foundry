...
export type RepoInventoryOutput = z.infer<typeof RepoInventoryOutputSchema>;

function toPosix(rel: string): string {
  return rel.split(sep).join("/");
}

function relFromRepo(repoPath: string, absPath: string): string {
  return toPosix(relative(repoPath, absPath));
}

function shouldSkipDir(relPosix: string): boolean {
  const parts = relPosix.split("/").filter(Boolean);
  if (parts[0] === ".foundry" && parts[1] === "out") return true;
...
function isExpoConfigFile(baseName: string): boolean {
  const b = baseName.toLowerCase();
  if (b === "app.json") return true;
  if (b.startsWith("app.config.") && /\.(js|ts|mjs|cjs)$/.test(b)) return true;
  return false;
}
...
async function hasExpoInPackageJson(repoPath: string, relPath: string): Promise<boolean> {
  const data = await readJsonSafe(join(repoPath, relPath));
  if (!data) return false;
  const merge = (o: unknown) =>
...
export async function scanRepo(repoPath: string): Promise<RepoInventoryOutput> {
  const generatedAt = new Date().toISOString();
  const tree: RepoInventoryOutput["tree"] = [];
  const languages = new Set<string>();
  const packageJsonPaths: string[] = [];
  const expoConfigPaths: string[] = [];
  const easJsonPaths: string[] = [];
  const readmePaths: string[] = [];
  let supabaseDirFromConfig: string | undefined;
  ...
  while (queue.length) {
    ...
    if (dirPosix && shouldSkipDir(dirPosix)) continue;
    ...
    for (const ent of entries) {
      ...
      if (ent.isDirectory()) {
        if (shouldSkipDir(posix)) continue;
      ...
      if (lower === "package.json") packageJsonPaths.push(posix);
      if (lower === "eas.json") easJsonPaths.push(posix);
      if (/^readme\.md$/i.test(base)) readmePaths.push(posix);
      ...
      if (isExpoConfigFile(base)) expoConfigPaths.push(posix);
    }
  }
  ...
  let repoTypeGuess: RepoInventoryOutput["summary"]["repoTypeGuess"] = "unknown";
  if (hasExpo) repoTypeGuess = "expo_rn";
  else if (hasPythonRoot || languages.has("python")) repoTypeGuess = "python";
  else if (packageJsonPaths.length > 0) repoTypeGuess = "node";

  const detected: RepoInventoryOutput["detected"] = {};
  if (await pathIsDir(join(repoPath, "apps", "mobile"))) detected.mobileAppDir = "apps/mobile";
  if (hasSupabaseFolder) detected.supabaseDir = "supabase";
  if (await pathIsDir(join(repoPath, "packages", "rules-engine"))) detected.rulesEngineDir = "packages/rules-engine";
...
function buildReadme(output: RepoInventoryOutput, projectName: string): string {
  ...
  if (d.mobileAppDir) lines.push(`- **Mobile app:** \`${d.mobileAppDir}\``);
  if (d.supabaseDir) lines.push(`- **Supabase:** \`${d.supabaseDir}\``);
  if (d.rulesEngineDir) lines.push(`- **Rules engine:** \`${d.rulesEngineDir}\``);
  if (!d.mobileAppDir && !d.supabaseDir && !d.rulesEngineDir) lines.push("- _(none of the common roots detected)_");

  lines.push("", "## Next stage inputs (suggestions)", "");
  lines.push(
    "- **market_gap_analysis:** Use `config.project` north star + differentiators; cross-check stack (`repoTypeGuess`) and whether mobile/Supabase are in play.",
  );
...
export const repoInventoryStage: Stage<StageInputComposition, RepoInventoryOutput> = {
  name: "repo_inventory",
  ...
  async run(ctx: RunContext, input: StageInputComposition): Promise<RepoInventoryOutput> {
