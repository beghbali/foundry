...
import { domainSummaryLine, getDomainKeyUserActions, getDomainPrimaryMetric, getDomainPrimaryUserAction, getDomainSuccessExamples, getDomainVocabulary, hasDomain, } from "@foundry/core/projectDomain";
...
function inferDomain(input) {
    ...
    const haystack = [
        ...
        .join(" ")
        .toLowerCase();
    if (/\b(garden|gardening|gardener|plant care|plants|vegetable bed|harvest|seed|soil)\b/.test(haystack)) {
        return "gardening";
    }
    return "generic";
}
function buildGardening(input, hasExpo, hasSupabase, complaints, gaps) {
        ...
        assumptions_to_break: [
            {
                ...
                why_outdated_now: "Encyclopedic content is free on YouTube, Reddit, and Google. Users drown in reference; they lack actionable, contextual guidance.",
                new_capability: "On-device context (location, season, garden setup) can generate personalized weekly action plans without encyclopedic browsing.",
                implication_for_product: "Ship a weekly action feed first. Delay library content until the core decision loop is sticky.",
            },
            {
                assumption: "Plant identification is the hard problem to solve.",
                why_outdated_now: "ID models (PictureThis, Google Lens) are increasingly commoditized. Identification alone does not retain users.",
                new_capability: "The value gap is downstream: turning diagnosis into a recovery workflow with concrete next steps, not just a species label.",
                implication_for_product: "Pair every diagnosis with a recovery plan rather than competing on ID accuracy alone.",
            },
            {
                assumption: "Gardeners will manually log everything if the UI is good enough.",
                why_outdated_now: "Manual journaling apps (Gardenize) see drop-off after the novelty fades. Logging must create visible value immediately.",
                new_capability: "Infer state from quick actions, photos, and reminders so the garden profile improves without explicit data entry.",
                implication_for_product: "Minimize required input. Treat every tap as a signal that improves recommendations.",
            },
            {
                assumption: "A broad feature set wins the gardening market.",
                why_outdated_now: `Competitors (${complaints.length ? topComplaints.join(", ") : "see market gap analysis"}) spread thin across planning, care, social, and commerce. None own the weekly decision moment.`,
                new_capability: "A narrow, opinionated product that owns 'what should I do this week' can outperform broader tools on retention and habit.",
                implication_for_product: "Resist feature sprawl. Win the weekly check-in before expanding surface area.",
            },
        ],
        wedge_moves: [
            {
                move: "Weekly personalized garden action plan",
                why_it_wins: "No competitor delivers a curated, season-aware weekly to-do that adapts to the gardener's actual setup. This creates the repeat visit.",
                moat_vector: "workflow",
                minimum_viable_proof: "50 gardeners use the weekly plan for 4 consecutive weeks with >60% task completion rate.",
            },
            {
                move: "Diagnosis-to-recovery workflow",
                why_it_wins: "Competitors stop at identification. A recovery plan with steps, timeline, and follow-up check turns a panic moment into long-term trust.",
                moat_vector: "workflow",
                minimum_viable_proof: "30 gardeners complete a recovery workflow; >50% report the issue improved within the suggested timeframe.",
            },
            {
                move: hasSupabase
                    ? "Outcome data loop via Supabase-backed garden profiles"
                    : "Outcome data loop from lightweight garden profiles",
                why_it_wins: "Every logged outcome improves future recommendations for the user and the cohort. This compounds into a data moat competitors cannot replicate without the same engagement.",
                moat_vector: "data",
                minimum_viable_proof: "Average garden profile has 5+ logged events after 4 weeks; recommendation acceptance rate trends upward.",
            },
            {
                move: hasExpo
                    ? "Mobile-first contextual capture (photo, quick-log, location)"
                    : "Low-friction contextual capture at the point of action",
                why_it_wins: "Gardening happens outdoors and in motion. Capturing state at the moment of action is harder to replicate than desktop dashboards.",
                moat_vector: "engineering",
                minimum_viable_proof: "Median time to log an action is <10 seconds; >70% of logs happen within 1 hour of the task.",
            },
        ],
        moat_hypotheses: [
            {
                hypothesis: "Localized outcome data creates a compounding recommendation advantage that new entrants cannot bootstrap.",
                mechanism: "Each gardener's logged outcomes (what worked, what failed, in which season/zone) feed a model that improves recommendations for similar gardeners. More users = better guidance = more users.",
                leading_indicators: [
            ...
            {
                hypothesis: "Owning the weekly decision moment creates a workflow lock-in that encyclopedic competitors cannot displace.",
                mechanism: "When gardeners build a weekly habit around Verdant's action plan, switching costs rise because their garden state, history, and personalized plan live in the app.",
                leading_indicators: [
            ...
            {
                hypothesis: "Recovery workflows drive trust that converts free users to paid.",
                mechanism: "Anxiety moments (sick plant, pest outbreak) are high-intent. Delivering a concrete fix builds outsized trust and willingness to pay for premium guidance.",
                leading_indicators: [
...
function buildGeneric(input, hasExpo, hasSupabase, complaints, gaps) {
    const project = input.config.project;
        ...
        assumptions_to_break: [
            {
                ...
                why_outdated_now: "Modern successful products ship one tight loop first and expand from retention, not from breadth.",
                new_capability: "A narrow product that solves one job exceptionally builds stronger word-of-mouth than a broad but shallow one.",
                implication_for_product: `Focus entirely on ${topGap.toLowerCase()} before adding adjacent features.`,
            },
            {
                assumption: "Users will configure the product to fit their needs.",
                why_outdated_now: `The top complaint is "${topComplaint.toLowerCase()}". Users abandon products that require upfront setup.`,
                new_capability: "Opinionated defaults and progressive disclosure can deliver value before users invest effort.",
                implication_for_product: "Ship strong defaults. Let personalization emerge from usage, not from settings screens.",
            },
            {
                assumption: "Growth comes from marketing spend.",
                why_outdated_now: "Products with strong outcome loops generate organic referrals that outperform paid channels at scale.",
                new_capability: "If the core loop delivers a visible, shareable win, users become the acquisition channel.",
                implication_for_product: "Design for shareable outcomes before investing in paid acquisition.",
            },
        ],
            ...
            : [
                {
                    ...
                    why_it_wins: "Users who see value in the first session are far more likely to return. Competing products require too much setup.",
                    moat_vector: "workflow",
                    minimum_viable_proof: "50 new users complete the core job in session one; >40% return within 7 days.",
                },
                {
                    move: hasSupabase
                        ? "User-context data loop via Supabase-backed profiles"
                        : "User-context data loop from lightweight profiles",
                    why_it_wins: "Every interaction improves future recommendations. This compounds into a data advantage.",
                    moat_vector: "data",
                    minimum_viable_proof: "Recommendation acceptance rate trends upward over 4 weeks of active usage.",
                },
                {
                    move: hasExpo
                        ? "Mobile-native contextual capture"
                        : "Low-friction capture at the point of action",
                    why_it_wins: "Capturing user context at the moment of need is an engineering moat that dashboard products cannot replicate.",
                    moat_vector: "engineering",
                    minimum_viable_proof: "Median interaction time for the core action is under 15 seconds.",
                },
            ],
        moat_hypotheses: [
            {
                hypothesis: "Usage data creates a compounding recommendation advantage.",
                mechanism: "Each user action feeds a model that improves guidance for similar users. More users = better product = more users.",
                leading_indicators: [
            ...
            },
            {
                ...
                mechanism: "When users build a habit around the product's core loop, their history and context make switching costly.",
                leading_indicators: [
...
export const firstPrinciplesStage = {
    name: "first_principles",
    description: "Derive assumptions to break, wedge moves, moat hypotheses, and a kill list from market gaps and repo context.",
    ...
    async run(ctx, input) {
        ...
        await writeStageMarkdown(ctx, "first_principles", "README.md", buildReadme(validated, input.config.project.project_name));
        return validated;
    },
};
