 ...
 */
async function ensureBootedIosSimulator(repoPath, config) {
...
async function collectImageFilesRecursive(dir, depth, out) {
    if (depth > MAESTRO_DEBUG_MAX_DEPTH)
        return;
    let names;
    try {
 ...
 */
async function collectQaScreenshots(repoPath, sinceMs) {
...
async function detectPackageManager(repoPath) {
    ...
    for (const [pm, lock] of ordered) {
        if (await fileExists(join(repoPath, lock)))
            return pm;
    }
    return "npm";
}
function resolvePackageJsonPath(repoPath, input) {
...
function maestroInstallTasks(command, flowPath) {
...
function resolveMaestroConfig(input) {
 ...
 */
function extractFailingTestDetails(summary) {
...
function estimateLintErrors(stdout, stderr) {
    const blob = `${stdout}\n${stderr}`;
    if (!blob.trim())
        return 0;
...
async function grepCount(repoPath, regex, exts) {
...
async function countLargeFiles(repoPath) {
    const prune = "\\( -path ./node_modules -o -path ./.git -o -path ./dist -o -path ./build -o -path ./.next \\)";
 ...
 */
async function readPreviousIndependentQa(repoPath, currentRunId) {
    ...
    for (const runId of runDirs) {
        ...
        try {
            ...
            isOuterCyclePreCursor = (manifest.stages ?? []).some((s) => s.stage === "builder" && s.status !== "skipped" && s.status !== "pending");
        }
        catch {
            // Older runs may not have a parsable manifest; treat as unknown and only
            // use as last-resort fallback below.
        }
        if (isOuterCyclePreCursor)
            return qaData;
        if (!fallback)
            fallback = qaData;
    }
...
function isLikelyMaestroEnvironmentFailure(stdout, stderr) {
    const t = `${stdout}\n${stderr}`.toLowerCase();
    return (/\b0 devices\b/.test(t) ||
...
function maestroOutputSuggestsMetroOrPackagerIssue(stdout, stderr) {
    const t = `${stdout}\n${stderr}`.toLowerCase();
    return (/\bmetro\b/.test(t) ||
        /\bpackager\b/.test(t) ||
...
function maestroShipBlockingManualHint(stdout, stderr) {
...
export const independentQaStage = {
    name: "independent_qa",
    description: "Validate codebase quality after the builder: tests, lint, typecheck, scans, score, and recommendation.",
    ...
    async run(ctx, input) {
        ...
        if (hasTestScript) {
            ...
            if (!testsPassed) {
                ...
                if (failingDetails.failingFiles.length > 0 || failingDetails.firstErrorLine) {
                    ...
                    blockers.push(`Test suite failed after builder changes.${fileList ? ` Failing files: ${fileList}${moreFiles}.` : ""}${errLine}`);
                }
                else {
        ...
        if (hasLintScript) {
            ...
            if (!lintPassed)
                autoFixable.push("Lint errors detected — builder will auto-fix on next cycle.");
        }
        else {
            checks.push({
                name: "lint",
        ...
        if (typecheckName) {
            ...
            if (!typecheckPassed)
                autoFixable.push("Typecheck errors detected — builder will auto-fix on next cycle.");
        }
        else {
            checks.push({
                name: "typecheck",
        ...
        if (maestro.enabled) {
            if (maestro.required && !maestro.pipelineCommand) {
                warnings.push("Maestro smoke is required but `qa_automation.maestro.pipeline_command` is unset. Bare `maestro test` often fails without a running Metro/packager; set `pipeline_command` to e.g. `npm run qa:device -- --debug-output .maestro-debug --flatten-debug-output`.");
            }
            ...
            if (!maestroExists) {
                ...
                manualTasks.push(`Manual repo/tooling setup: create or restore Maestro flows at ${maestro.flowPath}, then run: ${maestro.command} test ${quoteShell(maestro.flowPath)}`);
            }
                ...
                if (maestro.autoBootSimulator) {
                    ...
                    if (!boot.ok && boot.note !== "skipped: not darwin")
                        warnings.push(`Simulator auto-boot: ${boot.note}`);
                }
                ...
                if (!maestroPassed) {
                    ...
                    if (envOnly) {
                        warnings.push(message);
                        manualTasks.push("Boot an iOS Simulator (or connect a device), run `npx expo run:ios` (or your platform build), then re-run smoke (or use `qa_automation.maestro.pipeline_command` with a script that starts Metro).");
                    }
                ...
                if (!maestroCommand.available) {
                    ...
                    if (maestro.required) {
                        ...
                        warnings.push(`${message} Device smoke skipped.`);
                    manualTasks.push(...maestroInstallTasks(maestro.command, maestro.flowPath));
                }
                else {
                    if (maestroCommand.installedNow) {
                        manualTasks.push("Maestro CLI was auto-installed by Foundry. Restart your shell once so PATH is refreshed globally for future runs.");
                    }
                    ...
                    if (!maestroPassed) {
                        ...
                        if (envOnly) {
                            warnings.push(message);
                            manualTasks.push("Boot an iOS Simulator (or connect a device), run `npx expo run:ios` (or your platform build), then `maestro test` on the configured flow path.");
                        }
                        else if (maestro.required) {
                            blockers.push(message);
                            maestroFailureIsShipBlocking = true;
        ...
        if (regression)
            score -= 20;
        if (hasLintScript && !lintPassed)
            score -= 5;
        if (typecheckName && !typecheckPassed)
            score -= 5;
        if (maestro.enabled && maestroFailureIsShipBlocking)
            score -= 20;
        if (consoleCount > CONSOLE_SPAM_THRESHOLD)
            score -= 5;
        if (todoCount > TODO_SPAM_THRESHOLD)
            score -= 5;
...
function buildReadmeMarkdown(projectName, pm, packageJsonPath, out, prev) {
    const lines = [
    ...
    lines.push("## Blockers (must fix before release)", "", out.blockers.length ? out.blockers.map((b) => `- ${b}`).join("\n") : "_None — ready to ship._", "", "## Manual tasks (system / toolset / external)", "", out.manualTasks.length ? out.manualTasks.map((t) => `- ${t}`).join("\n") : "_None._", "", "## Screenshot artifacts", "", out.screenshotArtifacts.length ? out.screenshotArtifacts.map((s) => `- \`${s}\``).join("\n") : "_No QA screenshots were captured for this run._", "", "## Auto-fixable (builder handles these)", "", out.autoFixable.length ? out.autoFixable.map((a) => `- ${a}`).join("\n") : "_None._", "", "## Warnings", "", out.warnings.length ? out.warnings.map((w) => `- ${w}`).join("\n") : "_None._", "", "## Checks", "", ...out.checks.map((c) => [`### ${c.name}`, "", c.passed ? "Status: **passed**" : "Status: **failed**", "", c.details, ""].join("\n")), "## Test output (excerpt)", "", "```text", out.testSummary || "(empty)", "```", "", "---", "", "_Generated by Foundry `independent_qa` stage._");
    return lines.join("\n");
}
