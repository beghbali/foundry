...
import { BUILD_SPEC_MD_REL, BUILD_SPEC_REL, GrandWizardOutputSchema, STUCK_DROP_THRESHOLD, computeUpstreamFingerprint, droppedParentKey, isEnvironmentalWorkItem, isVagueDirective, readBuildSpecLedger, renderBuildSpecMarkdown, slugifySliceId, taskIsConcrete, writeBuildSpecLedger, } from "@foundry/core/buildSpec";
...
export { GrandWizardOutputSchema } from "@foundry/core/buildSpec";
...
function execShell(command, cwd, timeoutMs) {
    return new Promise((resolve) => {
        ...
        execFile(shell, ["-lc", command], { cwd, env: process.env, timeout: timeoutMs, maxBuffer: 8 * 1024 * 1024 }, (error, stdout, stderr) => {
            resolve({
                exitCode: error ? 1 : 0,
...
function mintParentId(text, index) {
    return `dir-${index + 1}-${slugifySliceId(text).slice(0, 24)}`;
}
function mintTaskId(parentId, sub) {
 ...
 */
function fuzzyDirectiveMatch(a, b) {
 ...
 */
function checkDirectiveCoverage(input, existing, ledger) {
    ...
    if (currentParents.length === 0)
        return { covered: false, total: 0, matched: 0 };
    let matched = 0;
    for (const parent of currentParents) {
        if (candidates.some((c) => fuzzyDirectiveMatch(parent.text, c)))
            matched++;
    }
    return { covered: matched === currentParents.length, total: currentParents.length, matched };
}
function upstreamFingerprintForInput(input) {
...
function collectParentDirectives(input) {
    const cc = ConvergenceContractOutputSchema.safeParse(input.convergenceContract);
    const pd = ProductDefinitionOutputSchema.safeParse(input.productDefinition);
    const fb = FeedbackAgentOutputSchema.safeParse(input.feedback);
    const inv = InvestorPanelOutputSchema.safeParse(input.investorPanel);
    ...
    if (input.investorRefinement?.directives?.length) {
        for (const d of input.investorRefinement.directives)
            push(d, 120, "investor_refinement");
    }
    if (inv.success) {
        for (const d of inv.data.combinedRefinementDirectives ?? [])
            push(d, 110, "investor_panel");
    }
    if (cc.success) {
        ...
        for (const item of cc.data.mvpBoundary.mustShip)
            push(item, 95, "convergence_must_ship");
    }
    if (pd.success) {
        for (const item of pd.data.scope.mustShip)
            push(item, 90, "product_must_ship");
    }
    if (fb.success) {
        for (const item of fb.data.items ?? []) {
            if (item.status !== "open" || !item.shouldImplement)
                continue;
...
function guessFilesForText(text, screens) {
 ...
 * check.
 */
function heuristicDecompose(parents, screens) {
    const tasks = [];
    const uncovered = [];
...
function buildHeuristicSpec(input, ledger) {
    ...
    const incompleteParents = parents.filter((p) => {
        if (addressedParentIds.has(p.id))
            return false;
        if (addressedParentTexts.has(p.text.trim().toLowerCase()))
            return false;
        ...
        // investor prefix".
        if (addressedParentTextsRaw.some((t) => fuzzyDirectiveMatch(p.text, t)))
            return false;
        // Likewise for permanently-dropped parents.
        if (droppedParentTextsRaw.some((t) => fuzzyDirectiveMatch(p.text, t)))
            return false;
    ...
    const cycleTheme = topParents[0]?.text.slice(0, 160) ??
    ...
    const parentDirectives = topParents.map((p) => ({
        id: p.id,
        source: p.source,
        text: p.text,
    ...
    if (uncovered.length > 0) {
        notes.push(`Heuristic could not anchor ${uncovered.length} directive(s) to code: ${uncovered.map((u) => u.id).join(", ")}. LLM decomposition recommended.`);
    }
...
function applyConcretenessFilter(spec) {
    ...
    const parentDirectives = spec.parentDirectives.map((p) => ({
        ...p,
    ...
    const newSlice = {
        ...slice,
        tasks: concreteTasks,
        acceptance: concreteTasks.length > 0
...
function mergeLlmDraft(heuristic, draft) {
    const heuristicPrimary = heuristic.slices[0];
    const llmPrimary = draft.primarySlice;
    const sanitizeId = (raw, fallbackParentId, idx) => {
        const trimmed = (raw ?? "").trim();
        const looksLikeSentence = trimmed.length === 0 || trimmed.length > 48 || /\s/.test(trimmed) || /[,.;:!?]/.test(trimmed);
        if (looksLikeSentence)
            return mintTaskId(fallbackParentId, idx + 1);
        if (trimmed.startsWith("task-") || trimmed.startsWith("dir-"))
            return trimmed;
        return `task-${slugifySliceId(trimmed)}`;
    };
    const fallbackParentId = heuristic.parentDirectives[0]?.id ?? "dir";
    const tasks = (llmPrimary?.tasks ?? heuristicPrimary.tasks).map((t, i) => ({
        id: sanitizeId(t.id, fallbackParentId, i),
        task: t.task.trim(),
    ...
    const parentDirectives = (draft.parentDirectives ?? heuristic.parentDirectives).map((p) => {
        const sourceRaw = p.source ?? "other";
        const allowed = [
            "investor_refinement",
            "investor_panel",
            "open_objection",
        ...
        const source = allowed.includes(sourceRaw)
            ? sourceRaw
            : "other";
        return {
            id: p.id,
...
function llmPrompt(input, heuristic, alreadyAddressed) {
    const addressedBlock = alreadyAddressed.length > 0
        ? [
  ...
  "notes": ["string"]
        ...
        }, null, 2).slice(0, 28_000),
    ].join("\n");
}
async function tryRunLlmGrandWizard(ctx, input, heuristic, alreadyAddressed) {
    ...
    try {
        const parsed = GrandWizardLlmDraftSchema.parse(JSON.parse(raw));
...
async function writeCanonicalBuildSpec(ctx, spec, projectName) {
...
async function readExistingBuildSpec(repoPath) {
    try {
 ...
 */
function carryForwardIncompleteSpec(previous, ledger) {
...
export const grandWizardStage = {
    name: "grand_wizard",
    description: "Decompose investor directives, convergence objections, and feedback into concrete code-anchored tasks (BUILD_SPEC) for Cursor. Skips regen when upstream is unchanged; persists task completion across cycles via BUILD_SPEC_LEDGER.",
    ...
    async run(ctx, input) {
        ...
        const coverageReuse = !fingerprintMatches && existingSpec ? checkDirectiveCoverage(input, existingSpec, ledger) : { covered: false, total: 0, matched: 0 };
        ...
        if (shouldReuse && existingSpec) {
            ...
            const cachedStreak = { ...(ledger.undecomposedParentStreak ?? {}) };
            const cachedDropped = {
                ...(ledger.droppedParents ?? {}),
            };
            const cachedNewlyDropped = [];
            const survivingParents = [];
            ...
            const carryWithDrops = cachedNewlyDropped.length > 0
                ? {
                    ...carry,
        ...
        if (addressedTexts.length > 0 && output.parentDirectives.length > 0) {
            const survivingParents = output.parentDirectives.filter((p) => !addressedTexts.some((t) => fuzzyDirectiveMatch(p.text, t)));
            ...
            if (droppedParents > 0) {
                ...
                const slice = output.slices[0];
                const survivingTasks = slice.tasks.filter((t) => t.decomposedFrom.length === 0 ? true : t.decomposedFrom.some((id) => survivingIds.has(id)));
                output = {
                    ...output,
                    parentDirectives: survivingParents,
        ...
        const updatedStreak = { ...(ledger.undecomposedParentStreak ?? {}) };
