...
function inferDomain(input: MonetizationInput): Domain {
  const haystack = [
...
function revenueMetricTarget(input: MonetizationInput): number | undefined {
  const m = input.config.metrics.metrics.find(
    (m) => /revenue|arr|mrr/i.test(m.key),
  );
  return m?.target;
}

function buildGardening(input: MonetizationInput): MonetizationOutput {
    ...
    entitlements: [
      {
        id: "pro",
        description:
          "Full access to unlimited weekly tasks, unlimited diagnosis workflows, multiple garden profiles, and full history.",
      },
    ],
    offerings: [
      {
    ...
    gates: [
      {
        ...
        freeLimit: { type: "count" as const, value: 1, period: "week" as const },
        requiresEntitlement: "pro",
        paywallMoment:
          "After the free user completes their 1 weekly task, show the paywall when they try to view additional recommendations.",
      },
      {
        feature: "diagnosis_workflow",
        freeLimit: { type: "count" as const, value: 2, period: "month" as const },
        requiresEntitlement: "pro",
        paywallMoment:
          "When a free user starts their 3rd diagnosis in a calendar month, show the paywall before results load.",
      },
      {
        feature: "garden_profiles",
        freeLimit: { type: "count" as const, value: 1, period: "month" as const },
        requiresEntitlement: "pro",
        paywallMoment:
          "When a free user taps 'Add another garden', show the paywall explaining that multiple gardens require Pro.",
      },
      {
        feature: "full_task_history",
        requiresEntitlement: "pro",
        paywallMoment:
          "Free users see only the current week's history. When they scroll past the fold into older history, show the paywall.",
      },
      {
        feature: "personalized_insights",
        requiresEntitlement: "pro",
        paywallMoment:
          "After 4 weeks of usage, surface a 'Your garden insights' card. Tapping it shows the paywall for free users.",
      },
    ],
    analyticsEvents: [
      {
...
function buildGeneric(input: MonetizationInput): MonetizationOutput {
  const projectName = input.config.project.project_name;
  const pid = projectName.toLowerCase().replace(/[^a-z0-9]/g, "_");
    ...
    gates: [
      {
        ...
        freeLimit: { type: "rate" as const, value: 3, period: "week" as const },
        requiresEntitlement: "pro",
      ...
      },
      {
        ...
        requiresEntitlement: "pro",
        paywallMoment:
          "Free users see only the last 7 days of history. Attempting to access older data shows the paywall.",
      },
      ...(workflows.length > 1
        ? [
            {
        ...
        : []),
      {
        ...
        requiresEntitlement: "pro",
        paywallMoment:
          "After enough usage data is collected, surface a 'personalized insights' prompt. Free users see the paywall on tap.",
      },
    ],
    analyticsEvents: [
      {
...
function buildReadme(
  ...
  investorRefinement?: z.infer<typeof InvestorRefinementContextSchema>,
): string {
  const yearlyMonthly = (output.pricing.yearlyUsd / 12).toFixed(2);
  const savingsPercent = Math.round(
    (1 - output.pricing.yearlyUsd / 12 / output.pricing.monthlyUsd) * 100,
  );
  ...
  if (revenueTarget !== undefined) {
    ...
    lines.push(
      "",
      `**Revenue target:** ${targetDisplay} (from metrics.yaml). At $${yearlyMonthly}/mo effective annual price, this requires ~${Math.ceil(revenueTarget / output.pricing.yearlyUsd)} annual subscribers or ~${Math.ceil(revenueTarget / 12 / output.pricing.monthlyUsd)} monthly subscribers.`,
    );
  }
  ...
  lines.push(
    ...
    "## Paywall moments",
    "",
    "Each gate has a specific moment when the paywall appears — always *after* the user has experienced value, never before.",
    "",
  );
  ...
  if (investorRefinement) {
    lines.push(
      "",
      "## Investor panel refinement",
      "",
      `Round ${investorRefinement.round} — incorporate into pricing, gates, and analytics events:`,
      "",
      ...investorRefinement.directives.map((d) => `- ${d}`),
      "",
      "_See also `product_definition` must-ship items updated in the same refinement round._",
      "",
    );
  }
...
export const monetizationArchitectStage: Stage<MonetizationInput, MonetizationOutput> = {
  name: "monetization_architect",
  description:
    "Design RevenueCat-ready monetization config: pricing, entitlements, offerings, gates, and analytics events.",
  inputSchema: MonetizationInputSchema,
  outputSchema: MonetizationOutputSchema,
  async run(ctx: RunContext, input: MonetizationInput): Promise<MonetizationOutput> {
    ...
    await writeStageMarkdown(
      ...
      "README.md",
      buildReadme(validated, input.config.project.project_name, revTarget, input.investorRefinement),
    );
