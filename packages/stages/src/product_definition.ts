...
export type ProductDefinitionOutput = z.infer<typeof ProductDefinitionOutputSchema>;

type PdInput = z.infer<typeof ProductDefInputSchema>;

function applyInvestorRefinement(
  out: ProductDefinitionOutput,
  ir: NonNullable<PdInput["investorRefinement"]>,
 ...
 */
function applyConvergenceContract(
  out: ProductDefinitionOutput,
  contract: NonNullable<PdInput["convergenceContract"]>,
): ProductDefinitionOutput {
  ...
  return {
    ...
    scope: {
      ...
      shouldShip: out.scope.shouldShip.filter(
        (s) => !contract.mvpBoundary.mustNotShipYet.some((p) => p.toLowerCase() === s.toLowerCase()),
      ),
      wontShip: Array.from(
        new Set([
...
type Domain = "gardening" | "web_data_platform" | "generic";
...
function inferDomain(input: PdInput): Domain {
  const haystack = [
  ...
  if (
    /\b(web_data_platform|data platform|cleanroom|clean room|snowflake|databricks|redshift|bigquery|warehouse|evidence pack|trust ledger|policy hash|agreement hash|overlap|cross-cloud|aws)\b/.test(
      haystack,
    )
  ) {
...
function getCapabilities(input: PdInput): RepoCapabilities {
...
function buildGardening(input: PdInput, caps: RepoCapabilities): ProductDefinitionOutput {
  ...
  const targetUser =
    "Home gardeners (beginner to intermediate) who want guided, season-aware recommendations instead of generic reference content. They garden in containers, raised beds, or small plots and check their phone weekly to decide what to do next.";
...
function buildWebDataPlatform(input: PdInput, caps: RepoCapabilities): ProductDefinitionOutput {
  const projectName = input.config.project.project_name;
  const northStar = input.config.project.north_star;
  const configDifferentiators = input.config.project.core_differentiators ?? [];
  ...
  return {
    ...
    targetUser:
      "Business partnership, measurement, data engineering, security, and legal teams that need Snowflake, Redshift, Databricks, BigQuery, or AWS data collaboration with auditable policy enforcement.",
    coreWorkflows: [
      {
    ...
    acceptanceCriteria: [
      {
        id: "AC-01",
        description:
          "A business user can create a collaboration, choose the overlap use case, register datasets, and reach agreement review without writing SQL.",
...
function buildGeneric(input: PdInput, caps: RepoCapabilities): ProductDefinitionOutput {
  const projectName = input.config.project.project_name;
  const northStar = input.config.project.north_star;
  const configDifferentiators = input.config.project.core_differentiators ?? [];
  const primaryLoop = input.flywheel.flywheel[0];
          ...
          ? [
              {
                ...
                steps: input.flywheel.flywheel[1].steps.map((s) =>
                  s.length > 120 ? s.slice(0, 117) + "..." : s,
                ),
...
function buildReadme(output: ProductDefinitionOutput, projectName: string): string {
  const lines = [
...
export const productDefinitionStage: Stage<PdInput, ProductDefinitionOutput> = {
  name: "product_definition",
  description:
    "Synthesize flywheel loops and repo capabilities into a concrete product definition with workflows, scope, and acceptance criteria.",
  inputSchema: ProductDefInputSchema,
  outputSchema: ProductDefinitionOutputSchema,
  async run(ctx: RunContext, input: PdInput): Promise<ProductDefinitionOutput> {
