...
export type FlywheelOutput = z.infer<typeof FlywheelOutputSchema>;

type FlywheelInput = z.infer<typeof FlywheelInputSchema>;
...
function slugMetricPrefix(projectName: string): string {
...
function primaryDifferentiator(input: FlywheelInput): string {
  return input.config.project.core_differentiators?.[0]?.trim() || "a sharply scoped core workflow";
}

function userNoun(input: FlywheelInput): string {
  const hay = `${input.config.project.north_star} ${input.config.project.project_name}`.toLowerCase();
  if (/\b(b2b|enterprise|team|org|company|business|operator|clinic|practice)\b/.test(hay)) return "teams";
  if (/\b(consumer|shopper|family|parent|patient|traveler|driver)\b/.test(hay)) return "people";
  return "users";
}

function productSurface(input: FlywheelInput): string {
  if (input.repoInventory.summary.hasExpo) return "the mobile experience";
  if (input.repoInventory.summary.repoTypeGuess === "node") return "the product surface";
  return "the product";
}

function firstCompetitorName(input: FlywheelInput): string {
...
function buildProjectContextualFlywheel(input: FlywheelInput): FlywheelOutput {
  ...
  const gap2 = clip(
    input.marketGap.gapsToExploit[1]?.gap ??
      input.marketGap.gapsToExploit[0]?.howWeWin ??
      "proving you win on speed, clarity, or outcomes versus the status quo",
    160,
  );
  const gapHow = clip(input.marketGap.gapsToExploit[0]?.howWeWin ?? "evidence and iteration on the core loop", 140);
  const complaint1 = clip(
    input.marketGap.commonComplaints[0]?.theme ?? "fragmented tools and unclear next steps",
    130,
  );
  const complaint2 = clip(
    input.marketGap.commonComplaints[1]?.theme ??
      input.marketGap.commonComplaints[0]?.whyItMatters ??
      "trust erodes when promises outrun what the product can demonstrate",
    130,
  );
...
function buildReadme(output: FlywheelOutput): string {
...
export const flywheelDesignerStage: Stage<FlywheelInput, FlywheelOutput> = {
  name: "flywheel_designer",
  description:
    "Turn project config, market gaps, repo signals, and wedge moves into business-specific flywheels and phased focus.",
  inputSchema: FlywheelInputSchema,
  outputSchema: FlywheelOutputSchema,
  async run(ctx, input): Promise<FlywheelOutput> {
    ...
    if (wedgeMoves.length > 0) {
      const wedgeFocus = wedgeMoves.slice(0, 3).map(
        (w) => `${w.move} (${w.moat_vector} moat): ${w.why_it_wins}`,
      );
